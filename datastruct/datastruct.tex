...
Line~\lnref{chk_resz_dist} then checks to see if a concurrent resize
...
line~\lnref{get_newbucket} selects the bucket corresponding to the key,
and line~\lnref{acq_newbucket} acquires the bucket's lock.
...
and one in the new \co{ht_bucket} structure to synchronize with
concurrent \co{hashtab_add()} and \co{hashtab_del()} invocations.
...
\co{hashtab_lock_mod()}.
Line~\lnref{relbkt0} releases the lock on the old \co{ht_bucket}
structure, which is the only such structure in the common case
where there is no resize operation in progress.
If line~\lnref{ifbkt1} determines that there was a resize operation
in progress, then line~\lnref{relbkt1} releases the lock on the new
\co{ht_bucket} structure.
