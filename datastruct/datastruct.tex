...
\co{ht_get_bucket()} on lines~10-25.
...
The \co{ht_get_bucket()} function handles hash-table selection, using
\co{ht_get_bucket_single()} to select the bucket within the proper table.
Line~16 selects the bucket corresponding to the hash in the current
...
Finally, line~23 returns a reference to the selected bucket.
...
the data structures from being freed during the traversal, and then
line~10 obtains a reference to the bucket in the current hash table
...
to the bucket corresponding to the specified key, which might well be
located in a new resized hash table.
...
If line~25 detects memory-allocation failure, lines~26 and~27 return
...
This RCU-protected resizable hash table performs and scales almost
as well as does its fixed-size counterpart.
...
pointers, which indicates that hystersis should be applied to avoid
over-aggressive resizing.
Growing the table by large factors can also help reduce resizing overhead.

The key point is that although the \co{hashtab} structure is non-partitionable,
it will normally be read-mostly, which opens the door to applying RCU
to avoid this structure being a performance and scalability bottleneck.
...
Of course, the pair of pointer sets does impose some memory overhead,
...
day of four-kilobyte address spaces.
...
address every byte of memory, but because the \co{ht_bucket} structure
is more than one byte long, the \co{->ht_resize_key} field must have
