...
\QuickQuizChapter{chp:Data Structures: Hash Tables}{Data Structures: Hash Tables}{qqzdatastruct}
%
...
This chapter has focused on hash tables, including resizable hash tables,
which are not fully partitionable.
Although there are many other data structures, this exposition of hash
tables is an excellent introduction to the many issues surrounding
high-performance scalable data access, including:
...
Hash tables continue to be the workhorse data structure for low-level
concurrency software, but as we have seen, the same desireable spreading
property provided by the hash function can result in scalability
limitations due to poor locality of reference.
In addition, use cases requiring ordered traversal are not well-served
by hash tables.
One way around these issues is provided by trees, which is the subject
of the next chapter.
