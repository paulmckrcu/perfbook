...
The strategy in the next section can often be used in these cases.
...
Section~\ref{sec:locking:Locking Hierarchies and Pointers to Locks}.
This practice is sometimes the right thing to do, but you should do
...
\QuickQuizAnswer{
	Because these same rules apply to interrupt handlers used in
...
However, this approach as a number of vulnerabilities:
...
One reason is that exact counters do not perform or scale well, as was
...
what the addition and deletion operations are using the exact count for.
...
\item	Determining when to resize the hash table.
	...
	maintained in a nicely partitioned per-chain basis.
...
It turns out that there is now a strong theoretical basis for the
...
Although it is all too easy to blame a poor choice of API on locking, it
is also not particularly useful to do so.
...
at a good parallel-ready API.

Times change, and code must change with them.
That said, there can be a huge number of users of a popular library,
...
or her own poor (though understandable) choices.

\subsection{Deadlock-Prone Callbacks}
...
and even harder, and perhaps impossible, to design a locking hierarchy.
...
synchronization tool, it works best when designed into an application
