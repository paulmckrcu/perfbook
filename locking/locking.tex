...
And just as sawing a board into smaller pieces results a greater
fraction of that board becoming sawdust, shorter lock-based critical
sections waste a greater fraction of the time.
...
where sawing only the one board that needs to be sawed instead of
unconditionally sawing all ten of them reduces the amount of sawdust
...
\item	Computer systems' synchronization ``saw blades'' are extremely
	thick, thus cutting a wide kerf through time, as can be seen
	by reviewing
