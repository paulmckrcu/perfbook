...
For different types of locks, it is helpful to have a
carefully considered hierarchy from one type to the next.
For many instances of the same type of lock, for example, a per-node
lock in a search tree, the traditional approach is to carry out lock
acquisition in order of the addresses of the locks to be acquired.
Either way, in large program, it is wise to use tools such as the Linux-kernel
\co{lockdep}~\cite{JonathanCorbet2006lockdep}
to enforce your locking hierarchy.

\QuickQuiz{
	Acquiring the locks in address order makes a lot of sense!!!
	But I looked at an implementation of the C++ \co{lock()} free
	function, which acquires all of the specified locks at one go,
	avoiding deadlock.
	Why didn't that implementation use this address-ordering trick?
}\QuickQuizAnswer{
	For one thing, it must interoperate with the C++ \co{unique_lock}
	class's \co{.lock()} member function, which acquires just the
	one lock.
	So if the user invokes the \co{unique_lock} class's \co{.lock()}
	member function twice so as to acquire two different locks
	within the same thread, and without releasing the first lock in
	the meantime, then the \co{lock()} free function must respect
	that ordering.
	And the user probably did not acquire those two locks in address
	order.

	Worse yet, in C++ it is possible to create wrapper classes,
	so that a given lock might have several wrappers, each at its
	own arbitrary address.
	This also defeats address-order acquisition of locks.

	So the \co{lock()} free function must use other tricks to acquire
	its list of locks in a deadlock-free fashion.\footnote{
		Kudos to \ppl{Anthony}{Williams} for explaining this
		situation to me.}
}\QuickQuizEnd
...
\QuickQuiz{
	...
	\item	If the caller has a signal handler that acquires
		...
		within a signal handler is a no-no in many
		...
		same lock in thread context, and also while holding any
		other locks acquired while that same lock is held.
	\end{enumerate}
}\QuickQuizEnd
...
\QuickQuiz{
	...
	thread will access invalid memory (either free or reclaimed).
}\QuickQuizEnd
...
function, where passing a pointer to a \apipx{pthread_mutex_t}
...
lock as an argument or as the return value, do yourself a favor and
...
This situation is not confined to locking.
\Cref{fig:locking:Saw Kerf}
shows how this same principle applies to the age-old activity of
sawing wood.
As can be seen in the figure, sawing a board converts a small piece of
that board (the width of the saw blade) into sawdust.
Of course, locks partition time instead of sawing wood,\footnote{
	That is, locking is temporal synchronization.
	Mechanisms that synchronize both temporally and spatially
...
And this situation underscores the importance of the
synchronization\-/granularity tradeoff discussed in
...
a lock and the data that it protects~\cite{RalfJung2021RustSafeSysProg}.
...
\cref{chp:Partitioning and Synchronization Design}, locking
