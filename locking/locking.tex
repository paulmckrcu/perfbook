...
Nevertheless, studying a sample implementation can be helpful,
and so this section reviews the implementation shown in
...
\QuickQuiz{}
	...
	Figure~\ref{fig:locking:Sample Lock Based on Atomic Exchange}?
\QuickQuizAnswer{
	This would be a legitimate implementation, but only as long as
	this store is preceded by a memory barrier.
	Because the \co{xchg()} operation returns a value, it
...
That said, atomic instructions are not strictly needed to
