...
This approach as been used in production.
...
Nevertheless, such study is left as an exercise for the reader.
...
In addition, tools for deadlock detection~\cite{JonathanCorbet2006lockdep},
lock acquisition/release balancing~\cite{JonathanCorbet2004sparse},
cache-miss analysis~\cite{ValgrindHomePage},
hardware-counter-based profiling~\cite{OProfileHomePage}, and
many more besides.
...
the library's locks are leaves in the enclosing program's locking
hierarchy.
...
The rule here is again to release all library-function locks before
calling back into application code.

Signal handlers pose similar difficulties.
If an application signal handler is invoked from a signal received within
the library function, deadlock can ensue just as if the library function
had called the signal handler directly.
Unfortunately, the library function does not have the option of releasing
locks when a signal is received.
Any one of the following rules can prevent problems in this case:

\begin{enumerate}
\item	Don't use signals.
	Unfortunately, library functions cannot enforce this rule on
	applications.
\item	Don't acquire locks from within signal handlers.
	This is excellent advice, but some systems to permit locks to
	be acquired from within signal handlers.
	And even if the normal system-provided locks are prohibited
	within signal handlers, clever programmers can easily
	(if unwisely) create locks from readily available atomic
	operations.
\item	Explicitly avoid signal-handler deadlocks as follows:
	\begin{enumerate}
	\item	If application invokes the library function from
	...
	\end{enumerate}
	These deadlock conditions can be avoided by using tools like
	the Linux kernel's lockdep lock dependency
	checker~\cite{JonathanCorbet2006lockdep}.
\end{enumerate}

Constructing parallel libraries using locking is possible, but clearly
not as easy as constructing a parallel application.
...
@@@
It turns out that designers of parallel libraries must also exercise
considerable care in designing the library's
APIs~\cite{HagitAttiya2011LawsOfOrder,PaulEMcKenney2011SNC}.
To see this, imagine a hash table insertion function that returned
the exact number of elements in the hash table after the insertion.
