...
\begin{verbatim}
 ...
 63 }
 64 
 65 unsigned long read_count(void)
 66 {
 67   int c;
 68   int cm;
 69   int old;
 ...
 72 
 73   spin_lock(&gblcnt_mutex);
 74   sum = globalcount;
 75   for_each_thread(t)
 76     if (counterp[t] != NULL) {
 77       split_counterandmax(counterp[t], &old, &c, &cm);
 78       sum += c;
 79     }
 80   spin_unlock(&gblcnt_mutex);
 81   return sum;
 82 }
\end{verbatim}
}
\caption{Atomic Limit Counter Add, Subtract, and Read}
\label{fig:count:Atomic Limit Counter Add, Subtract, and Read}
\end{figure}

Figure~\ref{fig:count:Atomic Limit Counter Add, Subtract, and Read}
...
Lines~65-82 show \url{read_count()}.
Line~72 acquires \url{gblcnt_mutex} and line~80 releases it.
Line~74 initializes local variable \url{sum} to the value of
\url{globalcount}, and the loop spanning lines~75-79 adds the
per-thread counters to this sum, isolating each per-thread counter
using \url{split_counterandmax} on line 77.
Finally, line~81 returns the sum.
