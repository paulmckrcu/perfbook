...
return a value between the value that an ideal counter would hace
...
The implementation is shows in
...
The \url{eventual()} thread waits an arbitrarly chosen one millisecond
between passes.
...
\QuickQuizAnswer{
	...
	value returned by \url{read_count()} will become more inaccurate.
} \QuickQuizEnd
...
\QuickQuizAnswer{
	...
	\url{eventual()} threads, each covering its own subset of
	the other threads.
	In even more extreme cases, a tree-like hierarchy of
	\url{eventual()} threads might be required.
} \QuickQuizEnd
...
\QuickQuiz{}
	...
	Figure~\ref{fig:count:Atomic Limit Counter Variables and Access Functions}?
\QuickQuizAnswer{
	There is only one \url{counterandmax} variable \emph{per thread}.
	Later, we will see code that needs to pass other threads'
	\url{counterandmax} variables to \url{split_counterandmax()}.
} \QuickQuizEnd
...
\QuickQuiz{}
	...
	\url{atomic_t}?
\QuickQuizAnswer{
	Later, we will see that we need the \url{int} return to pass
	to the \url{atomic_cmpxchg()} primitive.
} \QuickQuizEnd
...
shows the \url{add_count()}, \url{sub_count()}, and
\url{read_count()} functions.

Lines~1-32 show \url{add_count()}, whose fastpath spans lines~8-15,
with the remainder of the function being the slowpath.
Lines~8-14 of the fastpath form a compare-and-swap (CAS) loop, with
the \url{atomic_cmpxchg()} primitives on lines~13-14 performing the
actual CAS.
Line~9 splits the current thread's \url{counterandmax} variable into its
\url{counter} (in \url{c}) and \url{countermax} (in \url{cm}) components,
while placing the underlying \url{int} into \url{old}.
Line~10 checks whether the amount \url{delta} can be accommodated
locally (taking care to avoid integer overflow), and if not,
line~11 transfers to the slowpath.
Otherwise, line~11 combines an updated \url{counter} value with the
original \url{countermax} value into \url{new}.
The \url{atomic_cmpxchg()} primitive on lines~13-14 then atomically
compares this thread's \url{counterandmax} variable to \url{old},
updating its value to \url{new} if the comparison succeeds.
...
\QuickQuizAnswer{
	Replacing the \url{goto} with a \url{break} would require keeping
	a flag to determine whether or not line~15 should return, which
	is not the sort of thing you want on a fastpath.
	If you really hate the \url{goto} that much, your best bet would
...
\QuickQuiz{}
	...
	changed it!
\QuickQuizAnswer{
	Later, we will see how the \url{flush_local_count()} function in
	Figure~\ref{fig:count:Atomic Limit Counter Utility Functions}
	might update this thread's \url{counterandmax} variable concurrently
...
Figure~\ref{fig:count:Atomic Limit Counter Read} shows \url{read_count()}.
Line~9 acquires \url{gblcnt_mutex} and line~16 releases it.
Line~10 initializes local variable \url{sum} to the value of
\url{globalcount}, and the loop spanning lines~11-15 adds the
per-thread counters to this sum, isolating each per-thread counter
using \url{split_counterandmax} on line 13.
...
\caption{Atomic Limit Counter Utility Functions}
\label{fig:count:Atomic Limit Counter Utility Functions}
\end{figure}
...
\QuickQuiz{}
	...
	empties it?
\QuickQuizAnswer{
	This other thread cannot refill its \url{counterandmax}
	until the caller of \url{flush_local_count()} releases the
	\url{gblcnt_mutex}.
	By that time, the caller of \url{flush_local_count()} will have
	finished making use of the counts, so there will be no problem
	with this other thread refilling --- assuming that the value
	of \url{globalcount} is large enough to permit a refill.
} \QuickQuizEnd
...
\QuickQuizAnswer{
	...
	\begin{enumerate}
	\item	If \url{flush_local_count()}'s \url{atomic_xchg()} executes
		before the \url{split_counterandmax()} of either fastpath,
		then the fastpath will see a zero \url{counter} and
		\url{countermax}, and will thus transfer to the slowpath
		(unless of course \url{delta} is zero).
	\item	If \url{flush_local_count()}'s \url{atomic_xchg()} executes
		after the \url{split_counterandmax()} of either fastpath,
		but before that fastpath's \url{atomic_cmpxchg()},
		then the \url{atomic_cmpxchg()} will fail, causing the
		fastpath to restart, which reduces to case~1 above.
	\item	If \url{flush_local_count()}'s \url{atomic_xchg()} executes
		after the \url{atomic_cmpxchg()} of either fastpath,
		then the fastpath will (most likely) complete successfully
		before \url{flush_local_count()} zeroes the thread's
		\url{counterandmax} variable.
	\end{enumerate}
...
\QuickQuizAnswer{
	The caller of both \url{balance_count()} and
	\url{flush_local_count()} hold \url{gblcnt_mutex}, so
...
\QuickQuiz{}
	...
	the point of having separate REQ and ACK \url{theft} states?
...
\QuickQuizAnswer{
 ...
 17   }
	...
	value of \url{THEFT_READY}, and start stealing before the
...
\QuickQuizAnswer{
	...
	of its \url{countermax} variable unless it holds the
	\url{gblcnt_mutex} lock.
	...
	is not permitted to change its \url{countermax} variable.
...
\QuickQuizAnswer{
	...
	The caller of \url{flush_local_count()} has already invoked
	\url{globalize_count()}, so the check on line~28 will have
	succeeded, skipping the later \url{pthread_kill()}.
} \QuickQuizEnd
...
\QuickQuizAnswer{
	The \url{theft} variable must be of type \url{sig_atomic_t}
...
\caption{Signal-Theft Limit Counter Add and Subtract Functions}
\label{fig:count:Signal-Theft Limit Counter Add and Subtract Functions}
\end{figure}

Lines~1-36 of
Figure~\ref{fig:count:Signal-Theft Limit Counter Add and Subtract Functions}
shows the \url{add_count()} function.
The fastpath spans lines~5-20, and the slowpath lines~21-35.
Line~5 sets the per-thread \url{counting} variable to 1 so that
any subsequent signal handlers interrupting this thread will
set the \url{theft} state to ACK rather than READY, allowing this
fastpath to complete properly.
Line~6 prevents the compiler from reordering any of the fastpath body
to preceed the setting of \url{counting}.
Lines~7 and 8 check to see if the per-thread data can accommodate
the \url{add_count()} and if there is no ongoing theft in progress,
...
\QuickQuizAnswer{
	...
	upper limit being se to the largest value that the counter is
...
Line~1 write-acquires the lock and line~3 releases it.
...
\QuickQuizAnswer{
	...
	\item	There could be any number of devices, so that the
		...
		lack of arguments to functions like \url{do_io()}.
	...
	\item	The I/O might fail, and so \url{do_io()} will likely
		need a return value.
	...
	\item	Both \url{add_count()} and \url{sub_count()} can
...
\QuickQuizAnswer{
	...
	to the corresponding \url{__thread} variable.
} \QuickQuizEnd
