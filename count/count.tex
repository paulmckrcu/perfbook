...
Such circulation will take significant time, courtesy of the finite
speed of light and the non-zero size of atoms discussed in
\cref{sec:cpu:Hardware Free Lunch?}.
Given all of these limitations, the performance seen in
\cref{fig:count:Atomic Increment Scalability on x86}
might not seem so bad after all, but as fancifully depicted in
\cref{fig:count:Waiting to Count},
it will certainly \emph{feel} extremely bad to your users!
...
avoids the aforementioned delays.
Perhaps it is not possible to break the laws of physics,\footnote{
	But who knows?
	They are after all only empirical!}
but it is often possible to work with them rather than against them.
