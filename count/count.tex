...
\QuickQuizAnswer{
	...
	\co{counterandmax} variables to \co{split_ctrandmax()}.
} \QuickQuizEnd
...
\begin{verbatim}
  ...
  7 
  8   do {
  9     split_ctrandmax(&counterandmax, &old, &c, &cm);
 ...
 40 
 41   do {
 42     split_ctrandmax(&counterandmax, &old, &c, &cm);
 43     if (delta > c)
 44       goto slowpath;
...
\begin{verbatim}
  ...
  5   int old;
  6 
  7   split_ctrandmax(&counterandmax, &old, &c, &cm);
  8   globalcount += c;
  9   globalreserve -= cm;
