...
\item	Partial partitioning can be applied to code (as in the statistical
	counters' partitioned updates and non-partitioned reads), but also
	across time (as in limit counters running fast when far from
...
\item	Read-only code paths should remain read-only:  Spurious
	...
	and scalability.
\item	Judicious use of delay promotes performance and scalability.
