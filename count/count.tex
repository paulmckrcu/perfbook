...
other hand, use of atomic instructions reasonable simple.

\subsection{Atomic Limit Counter Implementation}
\label{sec:count:Atomic Limit Counter Implementation}

Unfortunately, when causing a given thread to give up its count, we
need to atomically manipulate both that thread's \url{counter}
and \url{countermax} variables.
The usual way to do this is to combine them into a single variable,
...
% @@@ how does this code violate the C standard?
% @@@ assumes 8 bits per byte, assumes bitwise assignment from unsigned to int.
