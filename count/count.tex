...
The \co{inc_count()} function shown on lines~5-8 is identical to its
counterpart in
Figure~\ref{fig:count:Array-Based Per-Thread Statistical Counters}.
...
cycles through all the threads, using the \co{atomic_xchg()} function
to remove count from each thread's local \co{counter}, adding the
...
\co{__get_thread_var()} primitive, which can be quite expensive
...
Otherwise, line~14 subtracts \co{delta} from \co{globalcount},
...
\QuickQuiz{}
	Why do \co{globalize_count()} to zero the per-thread variables,
...
Lines~9-19 show \co{balance_count()}, which can is, roughly speaking
...
of \co{counterandmax}, and line~8 defines \co{MAX_COUNTERMAX}, which
...
Lines~10-15 show the \co{split_counterandmax_int()} function, which,
when given the underlying \co{int} from the \co{atomic_t
counterandmax} variable.
...
Lines~17-25 show the \co{split_counterandmax()} function, which
picks up the underlying \co{int} from the specified variable
on line~21, stores it as specified by the \co{old} argument on
line~23, and then invokes \co{split_counterandmax_int()} to split
it on line~24.
...
Lines~27-33 show the \co{merge_counterandmax()} function, which
...
actual CAS.
Line~9 splits the current thread's \co{counterandmax} variable into its
...
using \co{split_counterandmax} on line 13.
...
\co{count_unregister_thread()}.
The code for \co{globalize_count()} is shown on lines~1-12, and
...
Lines~34-54 show the code for \co{balance_count()}, which refills
...
line~56 and the \co{count_unregister_thread()} function starting on
line~65.
...
Similarly, the structure of \co{sub_count()} on lines~38-71 is the same
