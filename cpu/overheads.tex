...
Here, ``same-CPU'' means that the same CPU now performing the CAS operation
...
fifteen nanoseconds, or more than thirty clock cycles.
...
compares the contents of the specified memory location to a specified
``old'' value, and if they compare equal, stores a specified ``new'' value,
...
that location could be loaded, that value incremented to produce the
new value.
...
An blind CAS involving CPUs in different cores but on the same socket
misses the cache consumes almost fifty
nanoseconds, or almost one hundred clock cycles.
...
A CAS operation consumes more than 400~nanoseconds, or almost
...
Spatial locality of reference is clearly extremely important.
