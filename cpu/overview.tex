...
textbooks~\cite{Hennessy2026},
scholarly papers, and training
materials~\cite{GuilhermeAmadio2026CPUHWopt}.
...
\label{sec:cpu:Interrupts}

Back in the mid-1900s, many computers computed in splendid isolation,
cranking out numerical results with little or no outside interference.
But here in 2026, such computers are the exception rather than the rule.
...
\cref{fig:cpu:CPU; Interrupted}.

\begin{figure}
\centering
\resizebox{3in}{!}{\includegraphics{cartoons/r-2026-CPU-track-meet-interrupt}}
\caption{CPU, Interrupted}
\ContributedBy{fig:cpu:CPU; Interrupted}{Melissa Broussard, remixed}
\end{figure}

The time taken is not just due to executing instructions in the interrupt
handler, but also the time required for the CPU to ``find a good stopping
point'' in the stream of instructions that it is currently executing.
The reason that this takes time is that the CPU might be in the midst
of executing hundreds of instructions.
When commencing the interrupt handler and when returning to the
interrupted stream of instructions, additional time might be taken
refilling the CPU's instruction-execution machinery.
Additional challenges are posed by CPU instructions that disable and
enable interrupts because when finding a good stopping point, that
stopping point must be within a region of code where interrupts are
enabled.
Of course, CPU designers work hard to mitigate these delays, but there
are limits.

For more information on the challenges inherent in CPU instruction
execution, see
