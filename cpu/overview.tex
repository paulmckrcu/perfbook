...
However, suppose we have either a program with many loops with small loop counts
or an object-oriented program with many virtual objects that
...
Wrong guesses can be very expensive because the CPU must discard the
results of any instructions that were executed speculatively based on
the wrong guess.
In addition, regardless of whether the CPU stalls or guesses,
the pipeline will empty and have to be refilled, leading
to pipeline stalls (or ``bubbles'') that can drastically reduce performance,
...
call them ``level-0 caches,'' and they are quite a bit bigger than 4KB.
