...
in the form of an additional data structure, the \co{struct} \co{bucket}.
...
For example, the Linux kernel provides \co{synchronize_rcu()} to
block until the end of a subsequent grace period (and other related primitives
as well),
while Hart et al.~\cite{ThomasEHart2006a} provide a \co{free_node_later()}
primitive that frees the specified data element after the end of a
subsequent grace period (in contrast to their \co{free_node()} primitive
