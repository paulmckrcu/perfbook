...
Although this might well be the best approach in some cases, we
can do better.

One of the simplest and most effective ways to partition a data structure
is to hash it.
...
In contrast to the dining philosophers solution,
this implementation is an example of ``vertical parallelism'' or
``pipelining'', given that data moves from one thread to another.

\QuickQuiz{}
	What would be an even better way of handling concurrency?
\QuickQuizAnswer{
	...
	double-ended queue to be used.
	This ``horizontal scaling''
	approach allows much better performance and scalability,
	and is especially attractive if there are multiple threads
	working either end of the queue, because in this case there
	are no ordering guarantees anyway.
} \QuickQuizEnd
...
The remainder of this chapter presents additional partitioning and
replication design tools.
