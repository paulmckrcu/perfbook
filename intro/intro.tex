...
\QuickQuizM{
	...
	than sequential programming, productivity is paramount and
...
One such machine was the CSIRAC, the oldest still-intact fully electronic
stored-program computer, which was put into operation in
...
Unfortunately, a system that performs optimally on the user~1's job is
unlikely to perform very well on user~2's job.
...
\item	Optimize the sequential application.
\end{enumerate}
...
If there is but one unpartionable work item, then it can be executed by
at most one CPU at a time, which is by definition sequential execution.
...
\begin{figure}
\centering
\resizebox{3in}{!}{\includegraphics{intro/coarsen}}
\caption{Coarse Partitioning Reduces Synchronization Overhead}
...
structures.
As a rough rule of thumb, you want to keep the smallest number of
partitions needed to meet your scalability goals, with each partition
thus being as large as possible.
...
is quite possible that the added synchronization overhead will
overwhelm any performance benefit that might have resulted from the
use of 64~CPUs instead of 16.

\QuickQuiz{
	That sounds like a difficult tradeoff.
	Just how are developers supposed to resolve it???
}\QuickQuizAnswer{
	Testing different partitioning schemes on the available hardware
	is often the best approach.
	But don't forget about the possibility of substituting replication
	for partitioning, as will be shown in
...
\subsection{Existing Sequential Designs}
\label{sec:intro:Existing Sequential Designs}

Existing designs of sequential programs can be a limiting factor,
especially given the many pressures to make minimal changes when
adapting systems to run on multi-core systems.
For example, consider the following hash-table API that might be used
in sequential code:
...
the corresponding element, or \co{NULL} if there is no such element.

The problem with this API is the exact counts returned by \co{hash_add()}
and \co{hash_del()}.
Although maintaining and returning these counts is almost free in
sequential code, doing so on a multi-core system can be extremely
expensive and can limit scalability, as discussed in
\cref{chp:Counting}.
One way to fix this parallel-programming issue is to have these
functions instead return approximate counts (again as discussed in
\cref{chp:Counting}) or to have them remain silent about the number
of elements, as discussed in \cref{sec:datastruct:Partitionable Data
Structures}.

There are of course any number of other design patterns that work well
in sequential code but not in concurrent code.
However, this example should give you a good idea what to look for.
