...
data structure.
Therefoe, if all 128 executing threads attempt to
...
The \url{__sync_synchronize()} primitive issuse a ``memory barrier'',
which constrains both the compiler's and the CPU's ability to reorder
operations.
% @@@ reference appropriate chapter/appendix.
...
Therefore, this section provides a mapping between the POSIX and
...
In general, use the simplest tool that will get the job done.
If you can, simply program sequentially.
If that is insufficient, try using a shell script to mediate parallelism.
If the resulting shell-script \url{fork()}/\url{exec()} overhead is too
large, try using the C-language \url{fork()} and \url{wait()} primitives.
If the overhead of these primitives is still too large, then you
might need to use the POSIX threading primitives, choosing the appropriate
locking and/or atomic-operation primitives.
If the overhead even of the POSIX threading primitives is too great,
then the primitives introduced in
Chapter~\ref{chp:defer:Deferred Processing} may be required.

However, it is necessary to make the right design choices as well as
