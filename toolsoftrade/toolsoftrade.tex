...
(\co{api-pthread.h}),
this \co{waitall()} function having semantics similar to the
...
It is critically important to note that the parent and child do \co{not}
share memory.
...
\co{pthread_create()} primitive, for example, as shown on line~15 of
...
The \co{pthread_join()} primitive, shown on line~19,  is analogous to
...
While holding the lock, lines~40-48 increment the shared variable \co{x},
...
The \co{gcc} compiler offers a number of additional atomic operations,
...
Figure~\ref{fig:toolsoftrade:Demonstration of Exclusive Locks}.
These last two primitives are not provided directly by \co{gcc},
...
For example, the \co{gcc} \co{__sync_} family of primitives all
...
\co{gcc} primitives to those used in the Linux kernel.
Exact mappings are not always available, for example, the Linux kernel
has a wide variety of locking primitives, while \co{gcc} has a number
...
\co{gcc}'s atomic operations can be emulated reasonably straightforwardly
using \co{cmpxchg()}.
