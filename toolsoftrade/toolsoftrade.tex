...
\Cref{sec:toolsoftrade:Shared-Variable Shenanigans}
describes (and provides examples for) additional problems caused by
plain accesses,
\cref{sec:toolsoftrade:A Volatile Solution,%
...
the compiler is within its rights to assume that any loaded-from variables
are not concurrently stored to by any other thread and that any stored-to
variables are neither loaded from nor stored to concurrently by any
other thread.
...
or shared-variable shenanigans.
Prevent this by informing the compiler of your intentions, as described
below.
...
\item[Load fusing] occurs when the compiler reuses the result of a prior
load from a given variable instead of re-executing that load.
...
\QuickQuiz{
	How does ``load fusing'' differ from ``load merging''?
}\QuickQuizAnswer{
	There is some variation in terminology.
	For example, some would say that load fusing is instead when
	the compiler uses a single instruction to load both of a pair
	of adjacent variables, and would then use load merging to mean
	reusing a prior load instead of
	reloading~\cite{PaulEMcKenney2026P3692R4/OOTAwithoutTrying}.

	But what is in a name?

	In this field, sometimes quite a bit of confusion!
}\QuickQuizEnd

For example, suppose that it is necessary to invoke a function
...
non-volatile atomic loads into a single non-volatile atomic load.
The resulting four back-to-back calls to \co{do_something_quickly()}
might result in a failure to meet loop-exit latency constraints after
...
Users of C++ or Rust atomics might be tempted to instead use these
language's non-volatile atomics, which often provide a nice compromise
...
executed up to four times between the time that some other thread stores
a non-zero value to \co{need_to_stop} and the time that the loop exits.
...
\QuickQuiz{
	...
	Because if \co{need_to_stop} is non-static and if
	\co{do_something_quickly()} was separately compiled, the compiler
	...
	So function inlining can be a wonderful optimization, but it
	can also inflict functionality and latency penalties on careless
	coders.
}\QuickQuizEnd
...
stores.
This permits the compiler to drop the first store to this variable, which
...
to indicate that \co{do_something_else()} is executing, and line~5 sets
...
Unfortunately, there are additional problems with the code in
\cref{lst:toolsoftrade:Fusing Stores},
...
of volatile and atomic operations, the compiler will assume that there
...
\QuickQuiz{
	This is ridiculous!!!
	The \co{barrier()} primitive constrains the compiler, but it
	does absolutely nothing to prevent reordering by the underlying
	hardware.
	In what alternate universe would this be at all useful???
}\QuickQuizAnswer{
	One such universe was described a couple of paragraphs ago,
	namely, where interrupts or POSIX signals are used to sample
	the \co{in_progress} variable.
	Perhaps you should re-read that text.

	But it is true that hardware reordering could cause problems if
	the \co{in_progress} variable was sampled by some other thread.
	In that case, you could use an atomic \co{xchg()} operation
	(which is fully ordered) in place of the \co{WRITE_ONCE()}.
	You would of course ignore the \co{xchg()} operation's return
	value.

	Let's see if you can figure out what that other thread should
	do for its ordered read of \co{in_progress}!
}\QuickQuizEnd
...
This permits compiler to insert an unconditional store and a compensating
...
(Linux kernel \apik{smp_wmb()}) would order the store, but not the
check load.
...
\cref{lst:toolsoftrade:Fusing Stores} where the
