...
invented stores, and store-to-load transformations,
...
In the spirit of ``every little bit helps'', here is a list of situations
...
\item	If a shared variable is only accessed by a given CPU or thread
	and by a signal or interrupt handler running in that CPU's
	or thread's context, then the handler can use plain loads and
	stores, as can any code that has prevented the handler from being
	invoked, that is, code that has blocked signals and/or interrupts.
