...
\item[Associativity:]
	...
	Fully associative caches have the advantage of eliminateing
...
\item[Cache Coherence:]
	...
	In constrast, the memory consistency model for a given machine
...
\item[Fully-Associative Cache:]
	A cache with only one set, so that it can hold any subset of
	memory that fits within its capacity.
\item[Grace Period:]
	Any RCU read-side critical section that began before the
	start of a given grace period is guaranteed to have
	completed before the end of that grace period.
	Most RCU implementations define a grace period to be a
...
\item[MESI Protocol:]
	The modified/exclusive/shared/invalid (MESI) cache-coherence protocol.
	This protocol is named after the states that the cache lines in
	a given cache can take on.
...
\item[Read-Copy Update (RCU):]
	...
	stale data can either be tolerated (as in routing tablees)
...
\item[Transactional Memory (TM):]
	...
	(hardwre transactional memory, or HTM), in software (software
	transcational memory, or STM), or in a combination of hardware
