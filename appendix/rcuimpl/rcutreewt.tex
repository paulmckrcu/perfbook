...
%DONE% __rcu_offline_cpu, rcu_offline_cpu (empty if !CONFIG_HOTPLUG_CPU)
%DONE% \url{__rcu_init()}.
%DONE%	rcu_online_cpu,
...
%DONE% simple functions: rcu_batches_completed, rcu_batches_completed_bh,
%DONE%	cpu_has_callbacks_ready_to_invoke, cpu_needs_another_gp,
%DONE%	rcu_get_root,

% GP detection.
%DONE%	note_new_gpnum, check_for_new_grace_period,
%DONE%	rcu_process_gp_end,
%DONE%	rcu_start_gp
%DONE%	rcu_check_quiescent_state()
%DONE%	cpu_quiet()
%DONE%	cpu_quiet_msk()
%DONE%	rcu_do_batch

%DONE% rcu_enter_nohz, rcu_exit_nohz, rcu_nmi_enter, rcu_nmi_exit,
%DONE%	rcu_irq_enter, rcu_irq_exit -- covered in formal.  Reference.
%DONE%	app:formal:Simplicity Avoids Formal Verification
%DONE%	Ditto dyntick_save_progress_counter, rcu_implicit_dynticks_qs,

%DONE% force_quiescent_state and friends. (partly done)
%DONE% Cover dyntick_record_completed, dyntick_recall_completed
...
%DONE% rcu_implicit_offline_qs (CONFIG_SMP only)
%DONE% rcu_process_dyntick
...
\item[\url{grpmask}:]
	...
	as suggested by Manfred Spraul @@@ citation @@@.
...
\QuickQuiz{Shouldn't line~42 of
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for rcutree rcu-check-callbacks}
...
\QuickQuiz{How does the code traverse a given path through
...
\QuickQuiz{Why not just expand \url{note_new_gpnum()} inline into
...
Line~10 invokes \url{cpu_needs_another_gp()} to see if this CPU really
...
\QuickQuiz{But there has been no initialization yet at line~15 of
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Starting a Grace Period}!
...
Line~38 sets local variable \url{rnp_end} to one past the last leaf
\url{rcu_node} structure, and line~39 sets local variable \url{rnp_cur}
to the first leaf \url{rcu_node} structure, so that the loop spanning
lines~40-44 traverses all leaves of the \url{rcu_node} hierarchy.
During each pass through this loop, line~41 acquires the current
leaf \url{rcu_node} structure's lock, line~42 sets the bits corresponding
...
\QuickQuiz{Why can't we merge the loop spanning lines~36-37 with
...
Line~45 then sets the specified \url{rcu_state} structure's \url{->signaled}
...
In this case, lines~11-12 invoke \url{cpu_quiet()} in order to report
this quiescent state to RCU.

\QuickQuiz{What prevents lines~11-12 of
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for rcu-check-quiescent-state}
...
\QuickQuiz{How do lines~22-23 of
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for cpu-quiet}
...
\QuickQuiz{Given that argument \url{mask} on line 2 of
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for cpu-quiet-msk}
...
\QuickQuiz{How do RCU callbacks on dynticks-idle or offline CPUs
	get invoked?}
\QuickQuizAnswer{
	They don't.
...
\QuickQuiz{Why would lines~14-17 in
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for rcu-do-batch}
...
\QuickQuiz{But how does the code in
	Figure~\ref{fig:app:rcuimpl:rcutreewt:NMIs from Dyntick-Idle Mode}
	handle nested NMIs?}
\QuickQuizAnswer{
...
\QuickQuiz{Why isn't there a memory barrier between lines~8 and 9 of
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for dyntick-save-progress-counter}?
...
\caption{Code for {\tt rcu\_implicit\_dynticks\_qs()}}
...
\QuickQuiz{Why wait the extra couple jiffies on lines~12-13 in
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for check-cpu-stall}?}
\QuickQuizAnswer{
...
\QuickQuiz{What prevents the grace period from ending before the
...
\QuickQuiz{Why does \url{print_other_cpu_stall()} in
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for print-other-cpu-stall}
...
% Stall stuff.
%DONE%	record_gp_stall_check_time
%DONE%	check_cpu_stall().
%DONE%	print_cpu_stall()
