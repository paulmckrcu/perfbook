...
\OriginallyPublished{Section}{app:rcuimpl:Preemptable RCU}{Preemptable RCU}{Linux Weekly News}{PaulEMcKenney2007PreemptibleRCU}

The preemptable RCU implementation is unusual in that
...
\QuickQuiz{Why is it important that blocking primitives
...
\QuickQuiz{Could the prohibition against using primitives
...
Each CPU also maintains \url{rcu_flip_flag} and
...
The \url{rcu_data} structure is a per-CPU structure, and
...
The \url{rcu_try_flip_waitack()} function, shown in
...
The \url{__rcu_advance_callbacks()} function, shown in
...
\QuickQuiz{How is it possible for lines~38-43 of
...
\QuickQuiz{What problems could arise if the lines containing
...
\QuickQuiz{What problems could arise in
...
Figure~\ref{app:rcuimpl:Preemptable RCU Worst-Case Scenario}.
...
\QuickQuiz{Suppose that the irq disabling in
...
\QuickQuiz{Why can't the \url{rcu_dereference()}
...
\QuickQuiz{What is a more precise way to say "CPU~0
