...
but rather for clarity and for instructional purposes.
...
for even these toy RCU implementations to be easily understood.
...
where it checks the bits indicated by \co{RCU_GP_CTR_NEST_MASK}
...
\QuickQuizE{
	...
	A fourth approach would be for \co{rcu_read_lock()} to recheck
	...
	A fifth approach is for the grace period process to wait for
	all readers to become aware of the new grace period.
	This works nicely in theory, but hangs if a reader blocks
	indefinitely outside of an RCU read-side critical section.

	A final approach is, oddly enough, to use a single-bit
	grace-period counter and for each call to \co{synchronize_rcu()}
	to take two passes through its algorithm.
	This is the approached use by userspace
	RCU~\cite{MathieuDesnoyers2009URCU}, and is described in
	detail in the journal article and supplementary
	materials~\cite[Appendix D]{MathieuDesnoyers2012URCU}.
}\QuickQuizEndE
}
...
\section{Fixed-Space RCU}
\label{sec:app:toyrcu:Fixed-Space RCU}

\Cref{sec:defer:Waiting for Readers}
described a class of RCU implementations where the grace period is a
fixed time interval.
\Cref{fig:defer:RCU Spatial/Temporal Synchronization} and
its discussion in
\cref{sec:defer:Quasi Reader-Writer Lock}
suggests that there might be a corresponding class of RCU implementations
where the grace period is not a fixed period of time, but instead a
fixed quantity of address space, and a 2026 discussion with \ppl{Hui}{Xie}
focused my attention on this possibility.

One can argue that the Linux-kernel address sanitizer
(KASAN)~\cite{AndreyRyabinin2015KASAN} incorporates fixed-space RCU\@.
KASAN keeps newly freed blocks of memory in a fixed-size quarantine.
This is done to increase the probability that a use-after-free access
occurs while the corresponding  block of memory has not yet been
reallocated, thus in turn reducing the number of false negatives that
could otherwise occur if freed memory was reallocated before the end of
life of any hanging pointer still (incorrectly) referencing it.
However, this can be thought of as a fixed-space RCU implementation,
where the ``readers'' (tasks with hanging pointers to newly freed blocks
of memory) are assumed to complete before the corresponding block of
memory passes through the fixed-sized quarantine and then is reallocated.

For its part, rcutorture does something similar by deferring freeing
of \co{RCU_TORTURE_PIPE_LEN} (that is, 10) of the blocks previously
referenced by the global variable \co{rcu_torture_current}.
In addition, there are 10 times that many elements total (that is,
100 of them).
As with KASAN, the idea is to reduce the incidence of false negatives,
so that an extremely long RCU reader combined with a series of extremely
short RCU grace periods would still have roughly a 99\pct\ chance of each
reader issuing a diagnostic for those too-short grace periods.

Although in these two examples, the intent of the fixed buffers is bug
detection, they could instead be used to create an extremely limited
and rather sharp-edged RCU grace-period detection mechanism.
So fixed-space RCU is quite real, although perhaps only very rarely
advisable.
...
\item	RCU read-side primitives should have $\O{1}$ computational
	...
	(But note that preemptible Linux-kernel RCU's
	\co{rcu_read_unlock()} can violate this desideratum for
	low-priority threads and at the end of extremely long-running
	RCU read-side critical sections.)
\item	RCU read-side primitives should be usable in all contexts (in
	the Linux kernel, they are permitted everywhere except in deep
	in the idle loop, from offline CPUs, and in kernel entry/exit
	code paths).
...
Those wishing more detail on requirements for
Linux-kernel RCU should refer to its internal
documentation~\cite{PaulEMcKenney2015rcu-requirements}.
