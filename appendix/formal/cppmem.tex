...
\section{Formal Verification and Memory Ordering}
\label{app:Formal Verification and Memory Ordering}
...
Unfortunately, few (if and) developers fully understand the complex
...
produced the CPPMEM tool based on the Power and ARM formalizations.
...
Section~\ref{sec:Anatomy of a Litmus Test}.
Section~\ref{sec:What Does This Litmus Test Mean?}
relates this litmus test to the equivalent C-language program,
and Section~\ref{sec:Running a Litmus Test} describes how to
apply CPPMEM to this litmus test.

\subsection{Anatomy of a Litmus Test}
\label{sec:Anatomy of a Litmus Test}
...
\caption{CPPMEM Litmus Test}
\label{fig:app:formal:CPPMEM Litmus Test}
\end{figure}

An example PowerPC litmus test for CPPMEM is shown in
Figure~\ref{fig:app:formal:CPPMEM Litmus Test}.
...
\QuickQuiz{}
	Why does line~8
	of Figure~\ref{fig:app:formal:CPPMEM Litmus Test}
...
\label{sec:What Does This Litmus Test Mean?}
...
\caption{Meaning of CPPMEM Litmus Test}
\label{fig:app:formal:Meaning of CPPMEM Litmus Test}
\end{figure}

Putting all this together, the C-language equivalent to the entire litmus
test is as shown in
Figure~\ref{fig:app:formal:Meaning of CPPMEM Litmus Test}.
...
\label{sec:Running a Litmus Test}
...
\caption{CPPMEM Detects an Error}
\label{fig:app:formal:CPPMEM Detects an Error}
\end{figure}
...
Figure~\ref{fig:app:formal:CPPMEM Detects an Error},
...
\caption{CPPMEM on Repaired Litmus Test}
\label{fig:app:formal:CPPMEM on Repaired Litmus Test}
\end{figure}

The fix to this Linux-kernel bug is to replace P0's \co{isync} with
\co{sync}, which results in the output shown in
Figure~\ref{fig:app:formal:CPPMEM on Repaired Litmus Test}.
...
\subsection{CPPMEM Discussion}
\label{sec:CPPMEM Discussion}
