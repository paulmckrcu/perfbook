...
\QuickQuiz{Why not simply increment \url{rcu_update_flag}, and then only
...
\QuickQuiz{But if line~7 finds that we are the outermost interrupt,
...
\QuickQuiz{Can you spot any bugs in any of the code in this section?}
\QuickQuizAnswer{
...
\QuickQuiz{Why isn't the memory barrier in \url{rcu_exit_nohz()}
...
\QuickQuiz{Isn't it a bit strange to model \url{rcu_exit_nohz()}
...
\QuickQuiz{Given there are a pair of back-to-back changes to
...
\QuickQuiz{But what would you do if you needed the statements in a single
...
\QuickQuiz{But what if the \url{dynticks_nohz()} process had
...
\QuickQuiz{Why are lines~45 and 46 (the \url{in_dyntick_irq = 0;}
...
\QuickQuiz{What property of interrupts is this \url{dynticks_irq()}
...
\QuickQuiz{Does Paul always write his code in this painfully incremental
	manner???}
\QuickQuizAnswer{
...
\label{fig:analysis:Memory-Barrier Fix Patch}
\end{figure}
...
\label{fig:analysis:Variable-Name-Typo Fix Patch}
\end{figure}
...
\item	{\bf Documenting code can help locate bugs}.
	...
	Figure~\ref{fig:analysis:Memory-Barrier Fix Patch}.
\item	{\bf Validate your code early, often, and up to the point
	...
	Figure~\ref{fig:analysis:Variable-Name-Typo Fix Patch}.
...
\item	{\bf The need for complex formal validation often indicates
	...
	And in fact much simpler design is presented in the following section.
\end{enumerate}

\subsection{Simplicity Avoids Formal Validation}
...
\label{fig:analysis:Variables for Simple dynticks Interface}
\end{figure}

Figure~\ref{fig:analysis:Variables for Simple dynticks Interface}
...
\QuickQuiz{But what happens if an NMI handler starts running before
...
\label{fig:analysis:Entering and Exiting Dynticks-Idle Mode}
\end{figure}

Figure~\ref{fig:analysis:Entering and Exiting Dynticks-Idle Mode}
