...
\item \verb|\co{}| for identifiers, \verb|\url{}| for URLs,
  ...
  Special cases for identifiers that are to be added to the Index of
  APIs are listed in
  \cref{sec:app:styleguide:Identifier}.
...
\begin{itemize}
\item	\verb|\apic{}| for C11 API members.
\item	\verb|\apig{}| for GCC API members.
\item	\verb|\apik{}| for Linux-kernel API members.
\item	\verb|\apikh{}| for Linux-kernel historical API members
  	such as \apikh{ACCESS_ONCE()}.
\item	\verb|\apialtc{}| for C11 API members, where the first argument
  	is printed and the second argument is indexed.
\item	\verb|\apialtg{}| for GCC API members, where the first argument
  	is printed and the second argument is indexed.
\item	\verb|\apialtk{}| for Linux-kernel members, where the first argument
  	is printed and the second argument is indexed.
\item	\verb|\apipf{}| for perfbook API members.
\item	\verb|\apipx{}| for pthreads API members.
\item	\verb|\apiur{}| for userspace-RCU API members.
\end{itemize}
