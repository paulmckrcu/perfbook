% appendix/srcu/srcu.c

\chapter{Sleepable Read-Copy Update}
\label{app:Sleepable Read-Copy Update}

\QuickQuizChapter{app:Sleepable Read-Copy Update}
...
Figure~\ref{fig:advsync:Sleeping While RCU Reading Considered Harmful},
...
\label{fig:advsync:Sleeping While RCU Reading Considered Harmful}
\end{figure}
...
\QuickQuiz{Why is sleeping prohibited within Classic RCU read-side
critical sections?}
\QuickQuizAnswer{
...
\QuickQuiz{Why not permit sleeping in Classic RCU read-side critical sections
...
\section{SRCU Implementation Strategy}
\label{sec:app:srcu:SRCU Implementation Strategy}
...
\subsection{Abolish Asynchronous Grace-Period APIs}
\label{sec:app:srcu:Abolish Asynchronous Grace-Period APIs}
...
\subsection{Isolate Grace-Period Detection}
\label{sec:app:srcu:Isolate Grace-Period Detection}
...
\section{SRCU API and Usage}
\label{sec:app:srcu:SRCU API and Usage}

The SRCU API is shown in Figure~\ref{fig:app:srcu:SRCU API}.
...
\caption{SRCU API}
\label{fig:app:srcu:SRCU API}
\end{figure}

\subsection{Initialization and Cleanup}
\label{sec:app:srcu:Initialization and Cleanup}
...
Section~\ref{sec:app:srcu:Cleaning Up Safely}.

\subsection{Read-Side Primitives}
\label{sec:app:srcu:Read-Side Primitives}
...
was described in Section~\ref{sec:app:srcu:Initialization and Cleanup},
...
\subsection{Update-Side Primitives}
\label{sec:app:srcu:Update-Side Primitives}
...
in Table~\ref{tab:app:srcu:SRCU Update and Read-Side Critical Sections}.
...
\label{tab:app:srcu:SRCU Update and Read-Side Critical Sections}
\end{table*}
...
\subsection{Cleaning Up Safely}
\label{sec:app:srcu:Cleaning Up Safely}
...
Figure~\ref{fig:app:srcu:SRCU Safe Cleanup} shows one way that cleanup
...
\label{fig:app:srcu:SRCU Safe Cleanup}
\end{figure}
...
\label{sec:app:srcu:Implementation}
...
\label{sec:app:srcu:Data Structures}

SRCU's data structures are shown in
Figure~\ref{fig:app:srcu:SRCU Data Structures},
...
\label{fig:app:srcu:SRCU Data Structures}
\end{figure}

\begin{figure}[htb]
\begin{center}
% \resizebox{3in}{!}{\includegraphics{appendix/srcu/srcuds}}
\includegraphics{appendix/srcu/srcuds}
\end{center}
\caption{SRCU Data-Structure Diagram}
\label{fig:app:whymb:SRCU Data-Structure Diagram}
\end{figure}

\subsection{Initialization Implementation}
\label{sec:app:srcu:Initialization Implementation}

SRCU's initialization function, \url{init_srcu_struct()}, is shown in
Figure~\ref{fig:app:srcu:SRCU Initialization}.
...
\label{fig:app:srcu:SRCU Initialization}
\end{figure}

SRCU's cleanup functions are shown in
Figure~\ref{fig:app:srcu:SRCU Cleanup}.
...
\label{fig:app:srcu:SRCU Cleanup}
\end{figure}

\subsection{Read-Side Implementation}
\label{sec:app:srcu:Read-Side Implementation}

The code implementing \url{srcu_read_lock()} is shown in
Figure~\ref{fig:app:srcu:Read-Side Acquisition}.
...
\label{fig:app:srcu:Read-Side Acquisition}
\end{figure}

The code for \url{srcu_read_unlock()} is shown in
Figure~\ref{fig:app:srcu:Read-Side Release}.
...
\label{fig:app:srcu:Read-Side Release}
\end{figure} 
...
\label{sec:app:srcu:Update-Side Implementation}
...
as can be seen in
Figure~\ref{fig:app:srcu:Update-Side Implementation}.
...
\label{fig:app:srcu:Update-Side Implementation}
\end{figure}

\QuickQuiz{Why is it OK to assume that updates separated by
...
\QuickQuiz{Why must line~17 in {\tt synchronize\_srcu()}
	(Figure~\ref{fig:app:srcu:Update-Side Implementation})
...
\section{SRCU Summary}
\label{sec:app:srcu:SRCU Summary}
