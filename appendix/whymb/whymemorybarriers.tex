...
with the invalidate queue, and \url{bar()} similarly had no reason
...
be guaranteed to be ordered unless there is an \url{ISB}
...
the load from \url{x} on line~1 to be ordered before the store to
\url{y} on line~4.
...
and the \url{ISB} instruction on line~6 ensures that
the load on line~7 happens after the load on line~1.
Note that inserting an additional \url{ISB} instruction somewhere between
...
does, with the exception that the Power \url{isync} instruction
is substituted for the ARM \url{ISB} instruction.
