...
\item	CPU~1 can now load the value of ``b'',
	...
	exits the \url{while} loop and proceeds
	to the next statement.
...
\QuickQuizAnswer{
	...
	of executing the assertion before the \url{while} loop completes.
	That said, some weakly ordered CPUs respect ``control dependencies.''
	Such CPUs would execute an implicit memory barrier after each
	conditional branch, such as the branch terminating the \url{while}
	loop.
	This example nevertheless uses an explicit memory barrier, as would
	be required on DEC Alpha.
} \QuickQuizEnd
...
\QuickQuiz{}
	If CPU~2 executed an \url{assert(e==0||c==1)} in the example in
	Table~\ref{tab:app:whymb:Memory Barrier Example 3},
...
\QuickQuiz{}
	Why is Alpha's \url{smp_read_barrier_depends()} an
	\url{smp_mb()} rather than \url{smp_rmb()}?
\QuickQuizAnswer{
	First, Alpha has only \url{mb} and \url{wmb} instructions,
	so \url{smp_rmb()} would be implemented by the Alpha \url{mb}
	instruction in either case.

	More importantly, \url{smp_read_barrier_depends()} must
...
\vspace{5pt}
	...
	Here the store to \url{p->hey_look} must be ordered,
	not just the loads from \url{p->a} and \url{p->b}.
} \QuickQuizEnd
...
(see Section~\ref{sec:app:whymb:POWER / Power PC}, but ARM uses a
different set of memory-barrier instructions~\cite{ARMv7A:2010}:

\begin{enumerate}
\item	\url{DMB} (data memory barrier) causes the specified type of
	...
	restricted to only writes (similar to the Alpha \url{wmb}
	and the POWER \url{eieio} instructions).
...
\item	\url{DSB} (data synchronization barrier) causes the specified
	type of operations to actually complete before any subsequent
	operations (of any type) are executed.
	The ``type'' of operations is the same as that of \url{DMB}.
	The \url{DSB} instruction was called \url{DWB} (drain write buffer
	or data write barrier, your choice) in early versions of the
	ARM architecture.
\item	\url{ISB} (instruction synchronization barrier) flushes the CPU
	pipeline, so that all instructions following the \url{ISB}
	are fetched only after the \url{ISB} completes.
	For example, if you are writing a self-modifying program
	(such as a JIT), you should execute an \url{ISB} after
	between generating the code and executing it.
\end{enumerate}

None of these instructions exactly match the semantics of Linux's
\url{rmb()} primitive, which must therefore be implemented as a full
\url{DMB}.
The \url{DMB} and \url{DSB} instructions have a recursive definition
of accesses ordered before and after the barrier, which has an effect
similar to that of POWER's cumulativity.

One difference between the ARMv7 and the POWER memory models is that
while POWER respects both data and control dependencies, ARMv7 respects
only data dependencies.
The difference between these two CPU families can be seen in the
following code fragment, which was discussed earlier in
Section~\ref{sec:app:whymb:Invalidate Queues and Memory Barriers}:
...
There is a control dependency between lines~10 and 11 in the above example.
This control dependency would cause POWER to insert an implicit memory
barrier between these two lines, but ARM would instead permit line~11
to be speculatively executed before line~10 completed.\footnote{
	Of course, as written above, the compiler would be within
	its rights to reorder lines~10 and 11.
	So please be careful out there!}
On the other hand, these two CPUs would both respect the data dependency
...
Both CPUs would place an implicit memory barrier between lines~5 and 6,
avoiding line~6 seeing a pre-initialization value for \url{p->a}.
Assuming that the compiler was prevented from reordering these two
lines, of course!
...
\ContributedBy{Figure}{fig:app:whymb:Half Memory Barrier}{Melissa McKenney}
\end{figure}
...
\subsection{POWER / Power PC}
\label{sec:app:whymb:POWER / Power PC}
...
x86~\cite[Section 8.2]{Intel64IA32v3A2009}, which mandates a total global order
