...
\chapter{Why Memory Barriers?}
\label{chp:app:whymb:Why Memory Barriers?}

\QuickQuizChapter{chp:app:whymb:Why Memory Barriers?}
...
\QuickQuiz{What happens if two CPUs attempt to invalidate the
same cache line concurrently?}
\QuickQuizAnswer{One of the CPUs gains access
...
\QuickQuiz{When an ``invalidate'' message appears in a
...
\QuickQuiz{If SMP machines are really using message passing
...
\item[Transition (f):]
	...
	this CPU's cache, which retains a read-only copy.
...
\QuickQuiz{How does the hardware handle the delayed transitions
	   described above?}
\QuickQuizAnswer{
...
Next CPU~3 loads some other cache line (at address~8),
which forces the data at address~0 out of its cache via a writeback,
...
\QuickQuiz{What sequence of operations would put the CPUs' caches
...
If one were foolish enough to use the very simple architecture we
have been discussing, one would be surprised.
A system with caches set up as shown in
Figure~\ref{fig:app:whymb:Caches With Store Buffers}
could potentially see the following sequence of events:
\begin{enumerate}
...
It is therefore violently counter-intuitive to software types, so much so
...
\item	CPU~1 executes \url{while (b == 0) continue}, but the cache line
	containing ``b'' is not in its cache.
	It therefore transmits a ``read'' message.
\item	CPU~0 executes \url{smp_mp()}, and marks all current store-buffer
...
involves lots of complex steps in silicon.
...
\item	CPU~0 receives the ``read'' message, and transmits the
	cache line containing the now-updated value of ``b''
	to CPU~1, also marking the line as ``shared'' in its own cache.
\item	CPU~1 receives the ``invalidate'' message for ``a'', places
	it into its invalidate queue, and transmits an ``invalidate
	acknowledge'' message to CPU~0.
	Note that the old value of ``a'' still remains in CPU~1's cache.
\item	CPU~1 receives the cache line containing ``b'' and installs
	it in its cache.
\item	CPU~1 can now finish executing \url{while (b == 0) continue},
	and since it finds that the value of ``b'' is 1, it proceeds
...
\item	CPU~1 processes the queued ``invalidate'' message, and
	invalidates the cache line containing ``a'' from its own cache.
	But it is too late.
\item	CPU~0 receives the ``invalidate acknowledge'' message for ``a''
	from CPU~0, and therefore applies
	the buffered store just in time to fall victim to CPU~1's
	failed assertion.
\end{enumerate}

Once again, the CPU designers cannot do much about this situation,
since the hardware does not know what relationships there might be
among what to the CPU are just different piles of bits.
...
Therefore, we can add a memory barrier as follows:
...
\item	CPU~1 can now finish executing \url{while (b == 0) continue},
	and since it finds that the value of ``b'' is 1, it proceeds
	to the next statement.
\item	CPU~1 executes the \url{smp_mb()}, marking the entry in its
	invalidate queue.
\item	CPU~1 executes the \url{assert(a == 1)}, but since there
	is a marked entry for the cache line containing ``a'' in the
	invalidate queue, CPU~1 must stall this load until that
	entry in the invalidate queue has been applied.
\item	CPU~1 processes the ``invalidate'' message, removing the
	cacheline containing ``a'' from its cache.
\item	CPU~1 is now free to load the value of ``a'', but since this
	results in a cache miss, it must send a ``read'' message to
	fetch the corresponding cache line.
\item	CPU~0 receives the ``invalidate acknowledge'' message for ``a''
	from CPU~0, and therefore applies
	the buffered store, changing the MESI state of the corresponding
	cache line to ``modified''.
\item	CPU~0 receives the ``read'' message for ``a''
	from CPU~1, and therefore changes the state of the corresponding
	cache line to ``shared'', and transmits the cache line to
	CPU~1.
\item	CPU~1 receives the cache line containing ``a'', and can therefore
	do the load.
	Since this load returns the updated value of ``a'', the
	assertion passes.
\end{enumerate}
...
\QuickQuiz{Does the guarantee that each CPU sees its own memory accesses
...
Portable code therefore cannot rely on this example coding technique,
although it will work on a number of real computer systems.

\QuickQuiz{Could this code be fixed by inserting a memory barrier
...
Portable code therefore cannot rely on this example coding technique,
although, as with the previous example, it will work on a number of
...
\QuickQuiz{Suppose that lines~3-5 for CPUs~1 and 2 are in an interrupt
...
\OriginallyPublished{Section}{sec:app:whymb:Memory-Barrier Instructions For Specific CPUs}{Memory-Barrier Instructions For Specific CPUs}{Linux Journal}{PaulMcKenney2005i,PaulMcKenney2005j}
...
This "just say no" approach can be eminently reasonable where it applies,
...
Although AMD64 is compatible with x86, it offers a stronger
slightly memory-consistency model, in that it will not reorder a store
ahead of a load~\cite{AMDOpteron02}.
After all, loads are slow, and cannot be buffered, so why reorder
a store ahead of a load?
Although it is possible in theory to create a parallel program that will
work on some x86 CPUs but fail on AMD64 due to this difference in
...
\label{fig:app:whymb:Half Memory Barrier}
\end{figure}
...
On the other hand, x86 CPUs give no ordering guarantees for loads, so
...
Note that some SSE instructions are weakly ordered ({\tt clflush}
