...
\QuickQ{What about correctness, maintainability, robustness, and so on???}
...
\QuickQ{And if correctness, maintainability, and robustness don't
...
\QuickQ{Given that parallel programs are much harder to prove
...
\QuickQ{What about just having fun???}
...
\QuickQ{Are there no cases where parallel programming is about something
...
\QuickQ{Given how cheap parallel hardware has become, how can anyone
...
\QuickQ{Some of the Quick Quiz questions seem to be from the viewpoint
...
\QuickQ{Wouldn't it be cleaner to acquire all the locks, and then
...
\QuickQ{Is the implementation shown in
	Figure~\ref{fig:defer:Per-Thread Lock-Based RCU Implementation}
	free from deadlocks?
	Why or why not?}
...
\QuickQ{But what if you hold a lock across a call to
...
\QuickQ{I thought that RCU read-side processing was supposed to
	be \emph{fast}!!!
...
\QuickQ{But don't we also need to check that a grace period is
...
\QuickQ{What happens if two CPUs attempt to start a new grace
...
\QuickQ{How does the code traverse a given path through
...
\QuickQ{C-preprocessor macros are \emph{so} 1990s!
...
\QuickQ{What happens if a CPU comes online between the time
...
\QuickQ{Why call \url{cpu_quiet()} on line~41 of
...
\QuickQ{But what if the \url{rcu_node} hierarchy has only a single
...
\QuickQ{Suppose that line~26 got executed seriously out of order in
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for rcu-offline-cpu},
...
\QuickQ{Given that an offline CPU is in an extended quiescent state,
	why does line~28 of
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for rcu-offline-cpu}
...
\QuickQ{But this list movement in 
	Figure~\ref{fig:app:rcuimpl:rcutreewt:Code for rcu-offline-cpu}
...
\QuickQ{Why not just expand \url{note_new_gpnum()} inline into
...
\QuickQ{But there has been no initialization yet at line~15 of
...
\QuickQ{Why can't we merge the loop spanning lines~36-37 with
...
\QuickQ{What prevents lines~11-12 of
...
\QuickQ{Given that argument \url{mask} on line 2 of
...
\QuickQ{How do RCU callbacks on dynticks-idle or offline CPUs
	get invoked?}
...
\QuickQ{Why would lines~14-17 in
...
\QuickQ{But how does the code in
...
\QuickQ{Why isn't there a memory barrier between lines~8 and 9 of
...
\QuickQ{Why wait the extra couple jiffies on lines~12-13 in
...
\QuickQ{What prevents the grace period from ending before the
...
\QuickQ{Why does \url{print_other_cpu_stall()} in
