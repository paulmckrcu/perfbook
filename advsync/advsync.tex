...
It is possible to create a fully non-blocking queue~\cite{MichaelScott96},
but these are more complex than the half-NBS algorithm outlined above.
The lesson here is to carefully consider what your real requirements are.
An irrelevant relaxation of requirements can often result in great
improvements in both simplicity and performance.

The opportunities for relaxation of requirements have increased based on
recent work indicates that in practice there is little practical
benefit from wait-free synchronization compared to non-blocking
synchronization~\cite{DanAlitarh2013PracticalProgress}.
The key requirement for these two classes of NBS to be equivalent
in practice is fair scheduling.

Interestingly enough, fair scheduling is but one constraint that is
often observed in practice.
Given fair locks that are granted to requesters in FIFO order at
a given priority level,
a method of avoiding priority inversion (such as priority
inheritance or priority ceiling), a bounded number of threads,
bounded critical sections,
bounded load,
and avoidance of fail-stop bugs,,
lcok-based application can provide deterministic
response times~\cite{BjoernBrandenbergPhD}.
...
Hopefully theoeretical frameworks continue to grow so as to better
