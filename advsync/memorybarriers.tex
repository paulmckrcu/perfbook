...
\label{lst:advsync:Message-Passing Litmus Test (No Ordering)}
\end{listing}

\paragraph{Load Followed By Load:}
Listing~\ref{lst:advsync:Message-Passing Litmus Test (No Ordering)}
...
\label{lst:advsync:Load-Buffering Litmus Test (No Ordering)}
\end{listing}

\paragraph{Load Followed By Store:}
Listing~\ref{lst:advsync:Load-Buffering Litmus Test (No Ordering)}
...
\label{lst:advsync:Message-Passing Litmus Test, No Writer Ordering (No Ordering)}
\end{listing}

\paragraph{Store Followed By Store:}
Listing~\ref{lst:advsync:Message-Passing Litmus Test, No Writer Ordering (No Ordering)}
...
\label{lst:advsync:Message-Passing Address-Dependency Litmus Test (No Ordering)}
\end{listing}

Listing~\ref{lst:advsync:Message-Passing Address-Dependency Litmus Test (No Ordering)}
...
Therefore,
Listing~\ref{lst:advsync:Message-Passing Address-Dependency Litmus Test (No Ordering)}'s
...
\subsubsection{Non-Multicopy Atomicity \& Cumulativity}
\label{sec:advsync:Non-Multicopy Atomicity and Cumulativity}

\begin{figure}[tb]
\centering
\resizebox{3.0in}{!}{\includegraphics{advsync/SystemArchBus}}
\caption{Global System Bus And Multi-Copy Atomicity}
\label{fig:advsync:Global System Bus And Multi-Copy Atomicity}
\end{figure}
...
shown in
Figure~\ref{fig:advsync:Global System Bus And Multi-Copy Atomicity}.
...
memory model to preserve multicopy atomicity$^{citation needed @@@}$,
...
\label{lst:advsync:WWC Litmus Test With Dependencies (No Ordering)}
\end{listing}

Listing~\ref{lst:advsync:WWC Litmus Test With Dependencies (No Ordering)}
...
\begin{figure}[tb]
\centering
\resizebox{3.0in}{!}{\includegraphics{advsync/NonMCAplatform}}
\caption{Shared Store Buffers And Multi-Copy Atomicity}
\label{fig:advsync:Shared Store Buffers And Multi-Copy Atomicity}
\end{figure}
...
Figure~\ref{fig:advsync:Shared Store Buffers And Multi-Copy Atomicity}.
...
\label{tab:advsync:Memory Ordering: WWC Sequence of Events}
\end{table*}

Table~\ref{tab:advsync:Memory Ordering: WWC Sequence of Events}
...
\label{lst:advsync:WWC Litmus With Release}
\end{listing}
