...
\begin{verbbox}
 ...
 7   int r2;
 8
 9   __atomic_store_n(x0, 2, __ATOMIC_RELAXED);
10   r2 = __atomic_load_n(x1, __ATOMIC_RELAXED);
11 }
12
13
14 P1(int *x0, int *x1)
...
16   int r2;
17
18   __atomic_store_n(x1, 2, __ATOMIC_RELAXED);
19   r2 = __atomic_load_n(x0, __ATOMIC_RELAXED);
20 }
21
...
\QuickQuiz{}
	Shouldn't lines~9 and~18 use \co{WRITE_ONCE()} and
	lines~10 and~19 use \co{READ_ONCE()} instead of this
	verbose \co{__atomic_} silliness?
\QuickQuizAnswer{
	In the Linux kernel, yes.
	But the \co{litmus7} tool used to run these litmus tests
	only knows about usermode primitives, in this case, gcc's
	C11-like intrinsics.
	They are not hard to figure out:
	Line~9 is more or less \co{WRITE_ONCE()} and
	line~10 is more or less \co{READ_ONCE()}.

	But you can use kernel primitives if you want.
	Just set up an environment where you can install and run
	kernel modules, then use the \co{klitmus} tool
	instead of the \co{litmus} tool.
} \QuickQuizEnd
...
11   r2 = __atomic_load_n(x1, __ATOMIC_RELAXED);
12 }
13
14
15 P1(int *x0, int *x1)
...
17   int r2;
18
19   __atomic_store_n(x1, 2, __ATOMIC_RELAXED);
20   __atomic_thread_fence(__ATOMIC_SEQ_CST);
21   r2 = __atomic_load_n(x0, __ATOMIC_RELAXED);
22 }
23
...
which has \co{__atomic_thread_fence()} directives\footnote{
	This is one of GCC's atomic intrinsics that was introduced
	briefly in
	Section~\ref{sec:toolsoftrade:Atomic Operations (C11)}.
	It is roughly similar to the Linux kernel's \co{smp_mb()} full
	memory barrier.}
placed between
...
\subsubsection{Address and Data Dependencies}
\label{sec:advsync:Address and Data Reordering}

\subsubsection{Control Dependencies}
\label{sec:advsync:Control Reordering}
