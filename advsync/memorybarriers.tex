...
\ContributedBy{Figure}{fig:advsync:CPUs Can Do Things Out of Order}{Melissa McKenney}
\end{figure}
...
that were produced in the 1990s.
...
\paragraph{Ears to Mouths.}
	...
	However, recent hardware would guarantee that at least one
...
\paragraph{Stores ``Pass in the Night''.}
	...
	on all 20\textsuperscript{th}-century systems.
	...
	accommodate ordering intuitions, and do permit this combination
	to be used safely.
...
the need for an explicit memory barrier in \url{spin_lock()}.
...
The \url{spin_lock()} primitive cannot proceed until
...
(which would be \url{B}) coming after the \url{LOAD} of \url{C}.
