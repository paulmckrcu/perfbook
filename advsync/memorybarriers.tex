...
\label{fig:advsync:CPUs Can Do Things Out of Order}
\end{figure}
...
For example, consider the following functions executing in parallel
...
\begin{verbatim}
 ...
 23 assert(b == 2);
\end{verbatim}
\label{minipage:advsync:Intuition Considered Harmful}
\end{minipage}
\vspace{5pt}
...
\QuickQuiz{How on earth could the assertion on line~21 of the code in
...
sequence of variables in a well-defined, global order.
...
\QuickQuiz{What assumption is the code fragment
...
Upon exit from the loop, \url{firsttb} will hold a timestamp
...
\QuickQuiz{How could CPUs possibly have different views of the
...
\QuickQuiz{Why do CPUs~2 and 3 come to agreement so quickly, when it
...
% Abandon all hope, ye who enter here! @@@  Too snarky?
...
\QuickQuiz{But if the memory barriers do not unconditionally force
...
\paragraph{Stores ``Pass in the Night''.}
	...
	happen first.
	\IfInBook{This strange (but quite common) effect is explained in
...
\QuickQuiz{How could the assertion {\tt b==2} on
...
\QuickQuiz{How could the code on 
...
\label{fig:advsync:Abstract Memory Access Model}
\end{figure}
...
\QuickQuiz{What effect does the following sequence have on the
...
\label{fig:advsync:Write Barrier Ordering Semantics}
\end{figure*}
...
\label{fig:advsync:Data Dependency Barrier Omitted}
\end{figure*}
...
\label{fig:advsync:Data Dependency Barrier Supplied}
\end{figure*}
...
\label{fig:advsync:Read Barrier Needed}
\end{figure*}

If, however, a read barrier were to be placed between the load of \url{B}
...
\label{fig:advsync:Read Barrier Supplied}
\end{figure*}
...
Even though the two loads of \url{A}
...
\label{fig:advsync:Read Barrier Supplied, Double Load}
\end{figure*}
...
\label{fig:advsync:Read Barrier Supplied, Take Two}
\end{figure*}
...
\QuickQuiz{What sequence of LOCK-UNLOCK operations \emph{would}
...
\QuickQuiz{What (if any) CPUs have memory-barrier instructions
...
\QuickQuiz{Given that operations grouped in curly braces are executed
...
\QuickQuiz{What are the constraints for 
	Table~\ref{tab:advsync:Lock-Based Critical Sections}?}
\QuickQuizAnswer{
	They are as follows:
	\begin{enumerate}
...
\label{fig:advsync:Memory Architecture}
\end{figure*}
...
\label{fig:advsync:Split Caches}
\end{figure*}
