...
Financial-services applications differ subtly from applications in
...
a few hundred microseconds if painstakingly configured, tuned, and run
...
advantages.)
The Linux 4.x and 5.x kernels can provide sub-hundred-microsecond
...
The \co{CONFIG_NO_HZ_FULL} approach focuses primarily on reducing jitter
from scheduling clock interrupts, which is the major concern for many
workloads.
However, ever-increasing CPU counts mean that it is increasingly
advantageous to run real-time and non-realtime applications on a single
system at the same time.
These non-realtime systems are likely to need to remap memory, and worse
yet the kernel may also need to remap kernel memory on their behalf.
This will result in TLB-flush IPIs being sent to all CPUs, including
the \co{nohz_full} CPUs, which will induce undesirable jitter on the
real-time workload.

These TLB-flush IPIs can be avoided on architectures such as ARM and
PowerPC that provide machine instructions that flush other CPUs'
TLBs, but even on these architectures, CPUs will still be subject
to IPIs due to scheduling events, the \co{smp_text_poke()} IPIs supporting
in-kernel self-modifying code, the Linux-kernel \co{smp_call_function()}
and \co{irq_queue_work()} primitives, and much else besides.
Schneider~\cite{ValentinScheider2025IPIinterference}
is working on IPIs that do not affect userspace execution, deferring
them until the next kernel entry.
This work currently features the \co{smp_text_poke()} IPIs supporting
in-kernel self-modifying code.
Work has also started on the more challenging IPIs flushing kernel TLBs,
which are still needed for systems not providing off-CPU TLB-flush
machine instructions.

However, even when the entire system is dedicated to the real-time
application, the most aggressive real-time applications would be best
served by deferring or otherwise preventing \emph{all} IPIs.
Zhou et al.~\cite{ZhouyiZhou2025deterministicsub05responselinux}
therefore propose placing yet more housekeeping responsibility onto
the real-time application.
In their approach, all IPIs to CPUs running real-time applications are
deferred until that application reaches a safe point where those IPIs'
latencies can be tolerated.
The application then calls into the kernel, which processes those
deferred IPIs.

This last approach might be criticized as having some serious usability
issues, but it achieves 0.5-microsecond latencies, which constitutes an
improvement of more than an order of magnitude over the best latencies
reported for Linux-kernel real-time systems.
For some users, this improvement will more than justify the additional
effort and care required by this approach.
Other users might wish to contribute to, or at least encourage,
Schneider's IPI-deferral work.
...
locking~\cite{PaulEMcKenney2007WhatIsRCUFundamentally,PaulMcKenney2012RCUUsage,PaulEMcKenney2024RCUAPI},
...
\QuickQuiz{
	But what if deterministic updates are also required?
	Doesn't this have a problem with non-deterministic RCU grace
	periods?
}\QuickQuizAnswer{
	It certainly has a problem with \co{synchronize_rcu()}, which
	blocks waiting for an RCU grace period to elapse, but there are
	other ways to handle grace periods.

	One approach is to instead use \co{call_rcu()}, which simply
	queues an RCU callback and continues execution.
	After a grace period has elapsed, this callback will be invoked,
	so that the RCU grace-period latency is offloaded from the
	thread doing the updates.
	However, some RCU implementations make \co{call_rcu()} do
	significant work, up to and including invoking arbitrary numbers
	of callbacks.
	Such implementations will require a different approach.

	And as of the early 2020s, one different approach within the Linux
	kernel is to use the polled RCU grace-period APIs described in
	\cref{sec:defer:RCU has a Family of Wait-to-Finish APIs}.
	Both the \apik{get_state_synchronize_rcu()} and the
	\apik{poll_state_synchronize_rcu()} APIs are non-blocking,
	which permits non-blocking algorithms to use RCU,
	at least as long as some other RCU function, such as
	\apik{start_poll_synchronize_rcu()}, is periodically invoked
	in order to ensure that the required RCU grace period is started.
	It is hoped that polled RCU grace-period APIs will appear in
