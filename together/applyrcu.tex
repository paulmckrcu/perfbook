...
\subsection{RCU Conversion Guide}
\label{sec:together:RCU Conversion Guide}

Converting code from reader-writer locking to RCU depends on the use
case, but generally requires the following steps:

\begin{enumerate}
\item	Apply \tco{rcu_dereference()} to pointer traversals and
	\tco{rcu_assign_pointer()} to pointer updates (or use higher-level
	list APIs),
\item	Once you have removed a node from its data structure,
	retire it, for example, pass it to \tco{call_rcu()} or invoke
	\co{synchronize_rcu()},
\item	If the node is passed to \co{call_rcu()}, add a \tco{rcu_head}
	structure to the node, and
\item	Review your code carefully to spot any reliance on mutual
	exclusion.
\end{enumerate}

Use cases that can tolerate the latency of a full grace period are
well-served by the example shown
in~\crefrange{lst:defer:Converting Reader-Writer Locking to RCU: Data}{lst:defer:Converting Reader-Writer Locking to RCU: Deletion}
starting on
\cpageref{lst:defer:Converting Reader-Writer Locking to RCU: Data}.
...
\FloatBarrier
\subsection{Fresh and Consistent Data}
\label{sec:together:Fresh and Consistent Data}

As noted in
\cref{sec:defer:RCU Usage Summary},
the data accessed by an RCU reader might be stale or inconsistent.
However, freshness and consistency can often be restored after the
fact.

For example, consider the RCU-protected pre-BSD routing structure
presented in
\cref{sec:defer:RCU for Pre-BSD Routing}.
Freshness and consistency can be restored by adding a
\co{false}-initialized \co{->removed} flag and a spinlock to each object.
During insertion, just before an object is made visible to RCU readers,
the \co{->removed} flag is initialized to zero and the spinlock is simply
initialized.
When an object is removed, its \co{->removed} flag is set to the value
one while holding its lock.
When a search finds an object, it acquires that object's lock.
If the \co{->removed} flag is set, the search simply releases the lock
and then proceeds as if the search had failed.

\begin{listing}
\input{CodeSamples/defer/route_cacm@data.fcv}
\caption{RCU Fresh-Data Routing Table Data Structures}
\label{lst:defer:RCU Fresh-Data Routing Table Data Structures}
\end{listing}

\begin{fcvref}[ln:defer:route_cacm:data]
For example, the \co{route_entry} structure in
\cref{lst:defer:RCU Fresh-Data Routing Table Data Structures}
can be modified to provide fresh and consistent data as shown by the
\co{->removed} field on \clnref{re_removed} and the \co{->route_entry_lock}
field on \clnref{re_lock}, both under \co{#ifdef FRESH}.
\end{fcvref}

\begin{listing}
\input{CodeSamples/defer/route_cacm@lookup.fcv}
\caption{RCU Fresh-Data Routing Table Lookup}
\label{lst:defer:RCU Fresh-Data Routing Table Lookup}
\end{listing}

\begin{fcvref}[ln:defer:route_cacm:lookup]
The \co{route_lookup()} function in
\cref{lst:defer:RCU Fresh-Data Routing Table Lookup}
can also be modified to provide fresh and consistent data as shown by
the code under \co{#ifdef FRESH}.
\Clnref{call_check} invokes the new \co{route_lookup_check} function
defined on \clnrefrange{rlc:b}{rlc:e}, passing it a pointer to the
current \co{route_entry} structure and also the current return value.
The return value from this function is used to update the \co{ret}
local variable, thus allowing this function to change the return value.

The \co{route_lookup_check()} function acquires the spinlock on
\clnref{rlc:acq}, and if \clnref{rlc:ckrem} determines that this element
has been removed, \clnref{rlc:retfail} sets the value of the local
variable \co{ret1} to \co{ULONG_MAX} to indicate lookup failure.
Of course, as soon \clnref{rlc:rel} releases the lock, some other thread
might invoke \co{route_del()} to remove this same object, thus rendering
the return value stale.
Therefore, any code requiring fresh and consistent data should be
placed in the \co{do_something_with()} function that is invoked on
\clnref{rlc:dsw}, where the object's \co{->route_entry_lock} will exclude
such removal.
Either way, \clnref{rlc:ret} returns either the passed-in interface or
the failure indication.

This listing's code structure was designed for ease of comparison with
\cref{lst:defer:RCU Pre-BSD Routing Table Lookup}.
Alert readers will note that this structure is not particularly good
for modularity, abstraction, or information hiding.
Such readers are encouraged to consider alternative APIs, perhaps passing
a function to \co{route_lookup()} or returning from \co{route_lookup()}
with the \co{->route_entry_lock} held.
\end{fcvref}

\begin{listing}
\input{CodeSamples/defer/route_cacm@add_del.fcv}
\caption{RCU Fresh-Data Routing Table Data Update}
\label{lst:defer:RCU Fresh-Data Routing Table Data Update}
\end{listing}

\begin{fcvref}[ln:defer:route_cacm:add_del]
For their part, the \co{route_add()} and \co{route_del()} functions
shown in
\cref{lst:defer:RCU Fresh-Data Routing Table Data Update}
can be easily modified to provide fresh and consistent data as shown by
the code under \co{#ifdef FRESH}.
In \co{route_add()}, \clnref{ra:irem} initializes the new \co{->removed}
field to zero and \clnref{ra:isl} initializes the new per-entry spinlock.
In \co{route_del()}, \clnref{rd:acq} acquires the mutex, \clnref{rd:rem}
sets the \co{->removed} field to the value one, and \clnref{rd:rel}
releases the mutex.
\end{fcvref}

This technique is used in the Linux kernel~\cite{Arcangeli03}, but it
is all to easy to overuse it.
Never forget that although holding the \co{->route_entry_lock} can
prevent the in-memory routing entry from changing, it can do absolutely
nothing to prevent changes in the external network configuration.
For example, if a ship's anchor severs an undersea fiber-optic cable,
re-routing of data will be required no matter how many systems hold
mutexes protecting the affected routing data.

Therefore, this technique tends not to be used in network routing, but
to instead in situations where the operations on the destination object
require a lock.
Examples include in-kernel operations on POSIX file descriptors and on
System-V semaphores.
In such cases, RCU provides fast and scalable traversal of the
search structure, and the per-object (as opposed to global) lock
provides the needed mutual exclusion, while also avoiding unnecessary
lock-contention-induced performance loss.
...
These situations can be handled in several ways:
\begin{enumerate*}[(1)]
\item	Use SRCU instead of RCU,
\item	Acquire a (possibly per-CPU) reference before exiting the RCU
	reader, or
\item	Acquire a hazard pointer before exiting the RCU reader.
\end{enumerate*}

\QuickQuiz{
	But if you are going to use hazard pointers anyway, why not just
	use them everywhere and dispense entirely with RCU?
}\QuickQuizAnswer{
	In many cases, switching completely to hazard pointers makes a
	great deal of sense.

	However, please keep in mind that hazard pointers cannot
	unconditionally traverse a hazard-protected pointer in all cases.
	In such cases, completely dispensing with RCU might greatly
	complicate the traversal of the linked data structure leading
	to the data element requiring long-term protection.
	The best strategy would then be to use RCU to traverse the linked
	data structure and hazard pointers to protect the data element
	at the end of this traversal.

	In contrast to other
	work~\cite{JeehoonKang2020MarryHazptrRCU},
	this method of marrying hazard pointers and RCU leverages the
	strengths of both mechanisms.
	Just be very careful to acquire the hazard pointer \emph{before}
	exiting the RCU read-side critical section!
}\QuickQuizEnd
...
The \co{txopt_get()} function in the v6.17 kernel takes this approach,
using \co{refcount_inc_not_zero()} to attempt to acquire the reference.

\subsubsection{Acquire a Hazard-Pointer Reference}
\label{sec:together:Acquire a Hazard-Pointer Reference}

Except that atomic operations such as \co{refcount_inc_not_zero()}
sometimes result in excessive memory contention.
In this case, one further fix is to use hazard pointers instead of an
...
\subsection{Lockless Configuration Update}
\label{sec:together:Lockless Configuration Update}

Suppose that you have a single global pointer that is either \co{NULL}
or points to a singleton data structure that provides configuration
information, similar to that discussed in
\cref{sec:defer:Minimal Insertion and Deletion}.
The twist being added in this section is allowing concurrent lockless
updates.

\begin{listing}
\input{CodeSamples/formal/herd/C-MP+o-xchg-rcusync-o+o-xchg-rcusync-o+rl-o-o-rul@whole.fcv}
\caption{Configuration-Update Example}
\label{lst:formal:Configuration-Update Example}
\end{listing}

At first glance, the updates in that section are lockless, given that
they use \co{rcu_assign_pointer()}.
...
%
\QuickQuizE{
	Do we really need \co{xchg()} in
	\cref{lst:formal:Configuration-Update Example}?
	Why not a simple load followed by a simple store?
}\QuickQuizAnswerE{
	Yes, we really do need that \co{xchg()}.
	If we used a simple load followed by a simple store, then another
	process could update the pointer between that load and that store,
	resulting in that other process's element being overwritten and
	leaked.
...
an atomic \co{xchg()} operation to carry out the update.
...
(\path{C-double-check-rcu-2.litmus}).\footnote{
	...
	\co{herd} takes about 20~minutes to analyze a four-process
...
\QuickQuiz{
	...
	It might well be.
	In which case, preallocation might be necessary, for example,
	in the Linux kernel, each CPU might be provided with a preallocated
	element.
}\QuickQuizEnd
...
In this case, we only ever need to set pointer~\co{p} the one time.
This means that if any would-be initializing process finds that
this pointer in non-\co{NULL}, it can simply abandon its attempt at
initialization.
...
\co{cmpxchg()} in \cref{lst:formal:Two-Process Double-Check CAS}.

\begin{fcvref}[ln:formal:C-double-check-cas-2:whole:P0]
\Clnref{load} fetches the pointer, and if \clnref{if} sees that it is
non-\co{NULL}, \clnref{deref} picks up the value.

Otherwise, \clnref{init:a} initializes an element and \clnref{cmpxchg}
attempts to install it at pointer~\co{p}.
If this attempt fails, it means that some other process has already done
the initialization, so \clnref{free} frees this process's element.
\end{fcvref}

\QuickQuiz{
	\begin{fcvref}[ln:formal:C-double-check-cas-2:whole:P0]
	Doesn't that \co{READ_ONCE()} on \clnref{load}
	\cref{lst:formal:Two-Process Double-Check CAS}
	need to be \co{smp_load_acquire()} in order to prevent early
	readers running concurrently with the initialization from seeing
	pre-initialization garbage?
	\end{fcvref}
}\QuickQuizAnswer{
	No, because \co{READ_ONCE()} is a volatile operation and therefore
	can head address dependency chains.
	In theory, another alternative is \co{rcu_dereference_raw()}.
	Except that this could be misleading because the element is
	never freed, not protected by \co{rcu_read_lock()}, and thus
	not really an RCU-protected pointer.

	For more information on address dependencies, please refer to
...
\QuickQuiz{
	Given an algorithm like that shown in
	\cref{lst:formal:Two-Process Double-Check CAS},
	why would anyone use double-checked locking?
}\QuickQuizAnswer{
	If the flag and data are carefully placed into the same cache
	line, then double-checked locking can offer better cache locality
	than can double-checked CAS\@.
	This is because in the double-checked CAS case, the pointer and
	the pointed-to element will almost always be in separate cache
	lines, potential resulting in two cache misses to double-checked
	locking's single cache miss.

	However, as the element grows in size, this advantage diminishes.
}\QuickQuizEnd

\subsection{Polling Patchups}
\label{sec:together:Polling Patchups}

Over a period of about ten years, Linux-kernel RCU has gained polling
grace-period APIs.
This section gives an overview of their use cases, focusing on RCU\@.
The material in this section also applies to SRCU via the
\apik{get_state_synchronize_srcu()}, \apik{start_poll_synchronize_srcu()},
and \apik{poll_state_synchronize_srcu()} APIs.

\subsubsection{Detecting Broken Readers}
\label{sec:together:Detecting Broken Readers}

One of the challenges with any scoped synchronization mechanism, whether
locking, interrupt disabling, or RCU, is that a called function might
momentarily exit the critical section.
Such exiting is considered bad practice, but there are situations where
it is necessary in order to avoid deadlocks, RCU CPU stall warnings, or
other consequences of remaining within the critical section for too long.
Except that such exiting can be a fatal surprise to the caller of the
function that momentarily exited the critical section.

\begin{listing}[tbp] 
{ \scriptsize
\begin{fcvlabel}[ln:together:Broken RCU Readers?]
\begin{VerbatimL}[commandchars=\\\[\]]
rcu_read_lock();					\lnlbl[rrl]
p = rcu_dereference(gp);				\lnlbl[rd]
suspicious_function();					\lnlbl[sf]
do_something_with(p);					\lnlbl[dsw]
rcu_read_unlock();					\lnlbl[rrul]
\end{VerbatimL}
\end{fcvlabel}
}
\caption{Broken RCU Readers?}
\label{lst:together:Broken RCU Readers?}
\end{listing}

\begin{fcvref}[ln:together:Broken RCU Readers?]
For example, see
\cref{lst:together:Broken RCU Readers?}.
\Clnref{rrl} enters an RCU read-side critical section, and \clnref{rrul}
exits it.
\Clnref{rd} loads a global pointer \co{gp} into local variable \co{p},
preparing it to be dereferenced.
\Clnref{sf} invokes a function that might possibly (and erroneously)
momentarily exit the critical section.
\Clnref{dsw} passes \co{p} to \co{do_something_with()}, which might
dereference this pointer, which could fail if \co{suspicious_function()}
had exited the critical section, even if only
momentarily~\cite{LorenzoStoakes2025ReaderLeak,VlastimilBabka2025PollReaderLeak}.

\end{fcvref}

\begin{listing}[tbp] 
{ \scriptsize
\begin{fcvlabel}[ln:together:Polling For Broken RCU Readers]
\begin{VerbatimL}[commandchars=\\\[\]]
rcu_read_lock();					\lnlbl[rrl]
oldstate = get_state_synchronize_rcu();			\lnlbl[gssr]
p = rcu_dereference(gp);				\lnlbl[rd]
suspicious_function();					\lnlbl[sf]
do_something_with(p);					\lnlbl[dsw]
WARN_ON_ONCE(poll_state_synchronize_rcu(oldstate));	\lnlbl[pssr]
rcu_read_unlock();					\lnlbl[rrul]
\end{VerbatimL}
\end{fcvlabel}
}
\caption{Polling For Broken RCU Readers}
\label{lst:together:Polling For Broken RCU Readers}
\end{listing}

\begin{fcvref}[ln:together:Polling For Broken RCU Readers]
Polled RCU grace periods provide at least some chance of detecting such
fatal surprises, as shown in
\cref{lst:together:Polling For Broken RCU Readers}.\footnote{
	Kudos to \ppl{Lorenzo}{Stoakes} and \ppl{Vlastimil}{Babka} for
	suggesting this approach.}
The \apik{get_state_synchronize_rcu()} and \apik{poll_state_synchronize_rcu()}
functions may be used to detect a momentary exit from the critical section,
as shown on \clnref{gssr,pssr}.\footnote{
	Kudos to Vlastimil Babka for inventing this technique.}
\Clnref{gssr} invokes \co{get_state_synchronize_rcu()}, placing its
return value in \co{oldstate}.
\Clnref{pssr} then passes \co{oldstate} to
\co{poll_state_synchronize_rcu()}, which returns \co{true}
only if a full RCU grace period has elapsed since the call to
\co{get_state_synchronize_rcu()}.
Assuming a correctly operating RCU implementation, this \co{true} return
can happen only if \co{suspicious_function()} exited the critical section,
hence the \apik{WARN_ON_ONCE()}.
\end{fcvref}

Unfortunately, RCU grace periods persist for many milliseconds, and
\co{suspicious_function()} might well exit the critical section for only
a handful of microseconds.
This means that the probability of the \co{WARN_ON_ONCE()} triggering is
quite tiny.
This probability can be increased by building the Linux kernel with
\co{CONFIG_RCU_STRICT_GRACE_PERIOD=y}, which speeds up RCU grace periods,
but restricts the number of CPUs to four.

It is also possible to expedite synchronous RCU grace periods booting the
Linux kernel with \co{rcupdate.rcu_expedited=1}.
In addition, the kernel address sanitizer (KASAN) can help locate
use-after-free bugs, including those induced by momentarily exiting
RCU read-side critical sections.

\subsubsection{Restraining Reclamation}
\label{sec:together:Restraining Reclamation}

RCU implementations typically invoke callbacks\footnote{
	From Linux-kernel \co{call_rcu()} or C++26 \co{rcu_retire()}.}
at a time of their choosing, which might or might not be at all convenient
for the workload.
This can be especially annoying for real-time workloads, for which
resuming a high-priority thread immediately after it is awakened will
usually be much more urgent than freeing a little memory.

In the Linux kernel, this situation can usually be handled with callback
offloading by building the kernel with \co{CONFIG_RCU_NOCB_CPU=y} and
setting the to-be-offloaded CPUs' bits in the \co{rcu_nocbs} kernel
boot parameter.
The system administrator can then set priorities and CPU bindings for
the resulting \co{rcuo} kernel threads, thus preventing these threads from
disturbing higher-priority threads or threads running on other CPUs.\footnote{
	See \url{https://lwn.net/Articles/988641/} for more information.}
This works, but requires global management of CPUs and priorities,
which is becoming increasingly difficult as larger and larger numbers
of workloads are consolidated onto a single system.
In addition, each individual RCU callback is invoked with softirq (also
known as ``bottom halves'') disabled, which means that a given callback
cannot be preempted even by the highest-priority thread, regardless of
whether that thread is running in the kernel or in userspace.

Another approach is for the in-kernel user to tag each data element
(or groups of them) with the return value from either
\apik{get_state_synchronize_rcu()} or \apik{start_poll_synchronize_rcu()},
then periodically call \apik{poll_state_synchronize_rcu()} when it is
a good time to free the memory.
Alternatively, the \co{_free()} counterparts of these three APIs may be
used, which consumes more memory, but more accurately accounts for both
normal and expedited RCU grace periods.

\subsubsection{Proactive Reclamation}
\label{sec:together:Proactive Reclamation}

There are sharp tradeoffs among the traditional RCU grace-period-wait
APIs.
On the one hand, \apik{synchronize_rcu()} incurs zero storage overhead,
but on the other hand it inflicts many milliseconds of latency on
its caller.
Although \apik{synchronize_rcu_expedited()} inflicts less latency
on its caller than does \co{synchronize_rcu()}, it still inflicts
tens of microseconds even on small systems, ranging beyond a
millisecond on larger systems.
For their part, \apik{call_rcu()} and \apik{kfree_rcu()} avoid this
latency, but at the expense of adding to pointers worth of \co{rcu_head}
to each RCU-protected data structure.
Finally, \apik{kfree_rcu_mightsleep()} avoids the \co{rcu_head} storage
overhead, and also avoids the multi-millisecond latency, but only if
there is ample free memory available.
When there is no free memory, \co{kfree_rcu_mightsleep()} inflicts just
as much latency on its caller as does \co{synchronize_rcu()}.

Those wanting the best of all possible worlds would of course advocate
for something that never inflicts any latency and that requires no pesky
\co{rcu_head} structures be added to RCU-protected data elements.

It \emph{is} possible to inflict minimal latency while also reducing the
per-data-element overhead (almost) to a single pointer by constructing a
singly linked list of data elements that have recently been removed from
an RCU-protected linked data structure, and then periodically passing
the full list to \co{call_rcu()}.
However, this means that a single RCU callback will be traversing and
and freeing each element of this list, thereby further increasing the
disruption that invoking callbacks inflicts on the workload.
The priority level of the disruption (though not its duration) can
be reduced by using \apik{queue_rcu_work()}, which invokes the callback
in a workqueue handler with preemption enabled.
Except that this can result in per-callback context switches, and even
ignoring that, the callbacks are being invoked at a time of RCU's and
the workqueue subsystem's choosing.

\begin{figure}[tbp]
\centering
\resizebox{3.0in}{!}{\includegraphics{together/PolledRCUcache}}
\caption{Polled RCU Cache}
\label{fig:together:Polled RCU Cache}
\end{figure}

This situation might seem grim, but there are use cases where the
RCU-protected data elements are linked into a search structure that
caches some fraction of the universe of such structures.
This is represented by the leftmost ``Accessible to readers'' box in
\cref{fig:together:Polled RCU Cache}.

Data elements that are unused for some time might be removed from the
reader-accessible cache and moved to a holding area (see the arrow
labeled ``A''), which is represented by the ``Old readers access''
box in the figure.
Such elements can be tagged with the return value from
one of the polled-grace-period API members, for example,
\co{get_state_synchronize_rcu()}.
Because there is only a limited number of polled values that can represent
a pending RCU grace period, this holding area could contain a small array,
each element of which contains the grace-period \co{oldstate} value.
The number of elements is specifed by \apik{NUM_ACTIVE_RCU_POLL_OLDSTATE}
and \apik{NUM_ACTIVE_RCU_POLL_OLDSTATE_FULL} macros.
Each entry of this array would head a list of elements that have been
removed from the left-most ``Accessible to readers'' list, but that might
still be referenced by old readers.
If a reader needs a data element that has been relegated to the
``Old readers access'' area, it can be once again made accessible to
readers, as indicated by the arrow at the top of the figure.

When one of the polling APIs (for example, \co{poll_state_synchronize_rcu()}
indicates that the corresponding RCU grace period has elapsed, then there
can be \emph{no} readers accessing them.
Those data elements are then moved to the ``No reader access'' area, as
indicated by the arrow labeled ``B''\@.
They can still be made accessible to readers, again as indicated by the
arrow at the top of the figure.

Finally, should memory run low, data elements can be freed, as indicated
by the arrow labeled ``C''.

This approach requires negligible additional per-data-element storage,
inflicts almost no latency on calling functions, and provides full
control of both the ``when'' and ``how'' of reclamation.

\subsubsection{Nurturing Non-Blocking Synchronization}
\label{sec:together:Nurturing Non-Blocking Synchronization}

As noted in the previous section, \apik{synchronize_rcu()} and
\apik{synchronize_rcu_expedited()} both inflict significant latencies
on their callers.
Although \apik{call_rcu()} and \apik{kfree_rcu()} inflict almost no
latency in the common case, in the Linux kernel \co{call_rcu()} can
inflict wake-up latencies and \co{kfree_rcu()} can inflict
memory-allocation latencies, both involving acquiring in-kernel locks.
Any of these latencies might be unacceptable for low-latency applications
that often choose to use non-blocking synchronization.

The in-kernel portions of such applications can use the polled APIs called
out earlier in this section, of which \apik{get_state_synchronize_rcu()},
\apik{get_state_synchronize_rcu_full()},
\apik{poll_state_synchronize_rcu()}, and
\apik{poll_state_synchronize_rcu_full()}
are wait-free.
None of these APIs will actually start a grace period,
however, any of the traditional grace-period-wait
APIs plus both \apik{start_poll_synchronize_rcu()} and
\apik{start_poll_synchronize_rcu_full()} may be invoked from some
latency-insensitive time or place to start the needed grace periods.
