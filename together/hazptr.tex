...
\subsection{Hazard-Pointer Conversion Guide}
\label{sec:together:Hazard-Pointer Conversion Guide}

Converting code from reference counting to hazard pointers requires
the following steps:

\begin{enumerate}
\item	Allocate and deallocate the hazard pointers themselves, which
	in the trivial implemention presented in
	\cref{sec:defer:Hazard Pointers}
	consists of selecting one of the two that are statically assigned
	to the current thread,
\item	Add a \tco{hazptr_head} structure to your hazard-pointer-protected
	nodes,
\item	Once you have removed such a node from its data structure,
	retire it, for example, pass it to \tco{hazptr_free_later()}, and
\item	Review your code carefully to spot any reliance on immediate
	detection of zero references.
\end{enumerate}

Explicit retirement is necessary because there is no efficient and
scalable way to determine precisely when the number of hazard pointers
referencing a given data element has dropped to zero.
