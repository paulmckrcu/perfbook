...
The \co{kref_put()} function on lines~16-28 checks for the counter having the
value ``1'' on line~22
(in which case no concurrent \co{kref_get()} is permitted),
or if atomically decrementing the counter results in zero on line~23.
In either of these two cases, \co{kref_put()} invokes the
specified \co{release} function and returns ``1'', telling the
caller that cleanup was performed.
Otherwise, \co{kref_put()} returns ``0''.
...
focuses on the aspects of \co{struct} \co{dst_entry} reference-count
