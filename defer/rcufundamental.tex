...
(\co{rcu_assign_pointer()} operation.
...
The \co{synchronize_rcu()} primitive might seem a bit
mysterious at first.
...
code in non-\co{CONFIG_PREEMPT} kernels!

There is a trick, and the trick is that RCU Classic read-side critical
sections delimited by \co{rcu_read_lock()} and
\co{rcu_read_unlock()} are not permitted to block or sleep.
...
meaning that \co{synchronize_rcu()} can safely return.

Thus, RCU Classic's \co{synchronize_rcu()}
...
Here, \co{run_on()} switches the current thread to the
specified CPU, which forces a context switch on that CPU.
The \co{for_each_online_cpu()} loop therefore forces a
...
words, for non-\co{CONFIG_PREEMPT} and \co{CONFIG_PREEMPT}
kernels, it does \emph{not} work for \co{CONFIG_PREEMPT_RT}
realtime (-rt) kernels.
...
The following text describes how to replace the \url{5,6,7} element
with \url{5,2,3} in such a way that any given reader sees one of these
two values.
