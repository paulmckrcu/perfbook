...
int read_gptr(int (*f)(struct route *rp))
{
  int ret = -1;
  struct route *rp;
...
shows how reading (\co{read_gptr()}),
...
However, this lock is not acquired by readers invoking \co{read_gptr()}:
...
But is this capability useful in practice?
...
in academic and industrial-research projects, as well as in production
by a number of userspace applications.
