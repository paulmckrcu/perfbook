...
RCU-protected objects from being freed.
...
delays~\cite{Balmau:2016:FRM:2935764.2935790,JeehoonKang2020MarryHazptrRCU}.
These defer the choice between hazard pointers and RCU to runtime,
and delegates responsibility for the decision to the software.
However, it is wise to remember that the
biggest advantage of hazard pointers over RCU is that a
hazard-pointers reader that maintains a long-term reference to a given
object does not prevent other objects from being freed.
In contrast, any long-running RCU reader will prevent all RCU-protected
objects from being freed.
On the other hand, the biggest advantage of RCU over hazard pointers is
that an RCU reader can unconditionally traverse an RCU-protected pointer.
In contrast, as with many reference-counting use cases, if the destination
object is concurrently removed, hazard pointers can (and must) refuse to
permit the traversal, in many cases requiring the traversal to restart
from the beginning.
These two aspects are mirror images of each other:
The reason that RCU can permit unconditional traversal is because any
RCU reader blocks freeing of all RCU-protected objects, and the reason
that hazard pointers can free any already-removed no-longer-protected
object that is not currently referenced by a hazard pointer is because
hazard pointers can refuse to permit a pointer traversal.
These mirror-imaged properties might explain why attempts to combine
hazard pointers and RCU into a single synchronization mechanism have
failed to catch on.
Successful innovations have instead refined each to accentuate its
strengths and minimize its weaknesses.
It also seems likely that use cases combining RCU and hazard pointers
will arise.
...
\QuickQuiz{
	Wait a minute!!!
	Linux-kernel RCU's \co{rcu_read_unlock()} can do lock
	acquisitions, and that cannot possibly be wait-free!
	What are you trying to pull???
}\QuickQuizAnswer{
	You seem to have missed the ``(QSBR \& urcu)'' in that cell.

	Non-preemptible Linux kernels use QSBR, in which
	\co{rcu_read_lock()} is not just bounded population-oblivious
	wait free, but executes zero instructions.

	Userspace RCU executes a finite sequence of simple instructions,
	thus also qualifying as bounded population-oblivious wait free.

	The one exception to this is the ``bullet-proof'' variant of
	userspace RCU, in which the first call to \co{rcu_read_lock()}
	registers the current thread as an RCU reader.
	But only the first call, so this added overhead can be rendered
	harmless by ensuring that each thread invokes \co{rcu_read_lock()}
	before undertaking any work requiring wait-free forward-progress
	guarantees.

	But what about preemptible Linux kernels?

	There, the goal is instead aggressive real-time response,
	which is covered in
	\cref{sec:advsync:Parallel Real-Time Computing}.
	
	In the meantime, please note that the Linux kernel supports
	only finite numbers of CPUs and that the lock acquisitions
	in \co{rcu_preempt_deferred_qs_irqrestore()}, which is
	called indirectly from \co{rcu_read_unlock()}, have
	interrupts disabled.
	In addition, those locks are replicated, so that a given lock
	is normally only acquired by one of sixteen CPUs.
	...
	users really need and then to implement it for them.
}\QuickQuizEnd
...
Furthermore, use of RCU polling APIs can provide a non-blocking
view of RCU grace periods, as discussed in
\cref{sec:together:Nurturing Non-Blocking Synchronization}.
