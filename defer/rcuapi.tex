...
\co{synchronize_net()}, wait for any type of currently executing
...
The ``SRCU: Sleeping readers'' column in
Table~\ref{tab:defer:RCU Wait-to-Finish APIs}
...
Linux has three variants of doubly linked list, the circular
...
be abbreviated in the same way lists are, as shown in
Figure~\ref{fig:defer:Linux Linked List Abbreviated}.

A variant of Linux's \co{hlist}, named \co{hlist_nulls}, provides multiple
distinct \co{NULL} pointer, but otherwise uses the same layout as shown in
Figure~\ref{fig:defer:Linux Linear Linked List (hlist)}.
In this variant, a \co{->next} pointer having a zero low-order bit is
considered to be a pointer.
However, if the low-order bit is set to one, the upper bits indentify
...
that reader with it.
The \co{is_a_nulls()} function returns true if passed an \co{hlist_nulls}
\co{NULL} pointer, and the \co{get_nulls_value()} function returns its
argument's \co{NULL}-pointer identifier.
...
This is also a storage-savings member, as it allows what would otherwise
be a separate spinlock to be stored with the pointer itself.

This background suffices to describe more complex updates in which an
