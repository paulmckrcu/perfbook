...
sections from blocking unrelated \co{synchronize_srcu()} invocations.
Of course, use of \co{synchronize_srcu()} in an SRCU read-side critical
...
asynchronous \co{call_srcu()} function, but special care must be taken
when using \co{call_srcu()}.
...
@@@ \co{hlist_bl} primitives and locking conventions.
