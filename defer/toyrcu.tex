...
so the \co{rcu_read_lock()} and
\co{rcu_read_unlock()} primitives no longer perform atomic operations.
...
with \co{rcu_idx} now being a \co{lock} instead of an
\co{atomic_t}.
...
Section~\ref{app:rcuimpl:Preemptible RCU} shows
...
The \co{synchronize_rcu()} primitives is still expensive,
...
straightforward.
Line~3 simply adds one to the global free-running \co{rcu_gp_ctr}
...
Line~10 then copies the \co{rcu_gp_ctr} global variable to the
...
Line~18 acquires the \co{rcu_gp_lock} (and line~28 releases it)
in order to prevent multiple
\co{synchronize_rcu()} instances from running concurrently.
Line~19 then increments the global \co{rcu_gp_ctr} variable by
...
so that lines~21-27 scan the \co{rcu_reader_gp} values until they
all are either even (line~22) or are greater than the global
\co{rcu_gp_ctr} (lines~23-24).
...
Interestingly enough, the implementation of \co{rcu_read_unlock()}
...
The first is that line~29 adds \co{RCU_GP_CTR_BOTTOM_BIT}
...
\co{rcu_quiescent_state()}, shown from lines~9-15 in the figure.
Threads entering extended quiescent states (for example, when blocking)
may instead use the \co{thread_offline()} and \co{thread_online()}
APIs to mark the beginning and the end, respectively, of such an
extended quiescent state.
As such, \co{thread_online()} is analogous to \co{rcu_read_lock()}
and \co{thread_offline()} is analogous to \co{rcu_read_unlock()}.
