...
\QuickQuiz{How can the grace period possibly elapse in 40 nanoseconds when
...
\QuickQuiz{If any even value is sufficient to tell \url{synchronize_rcu()}
...
\QuickQuiz{Why are the memory barriers on lines~17 and 29 of
	Figure~\ref{fig:defer:Free-Running Counter Using RCU}
	needed?
...
\QuickQuiz{Couldn't the update-side optimization described in
...
\QuickQuiz{Why not simply maintain a separate per-thread nesting-level
...
\QuickQuiz{Given the algorithm shown in
	Figure~\ref{fig:defer:Nestable RCU Using a Free-Running Counter},
...
\QuickQuiz{Again, given the algorithm shown in
	Figure~\ref{fig:defer:Nestable RCU Using a Free-Running Counter},
...
\QuickQuiz{Doesn't the additional memory barrier shown on line~19 of
	Figure~\ref{fig:defer:RCU Read Side Using Quiescent States},
...
\QuickQuiz{Why are the two memory barriers on lines~30 and 31 of
	Figure~\ref{fig:defer:RCU Read Side Using Quiescent States}
	needed?}
\QuickQuizAnswer{
...
an overhead of less than a tenth of a nanosecond.
The \url{synchronize_rcu()} overhead ranges from about half a microsecond
on a single-CPU Power5 system up to more than 100 microseconds on
a 64-CPU system.

\QuickQuiz{To be sure, the clock frequencies of ca-2008 Power
	systems were quite high, but even 5GHz doesn't permit
	loops to be executed in a tenth of a nanosecond!
...
\QuickQuiz{Why would the fact that the code is in a library make
