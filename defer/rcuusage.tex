...
\begin{figure}
\centering
\resizebox{\columnwidth}{!}{\includegraphics{defer/RCUphasedstatechange}}
...
\co{false} value of \co{be_careful} (and thus which might have invoked
...
logging of changes.
This use case can also be thought of as a pure use of the publish/subscribe
mechanism.
...
Therefore, most RCU-protected data structures provide for deletion.
...
covered in \cref{sec:defer:Add-Only List}.
...
If \clnref{nullchk} determines that it is \co{NULL}, \clnref{retfail}
returns failure.
...
providing full memory ordering both before and after the \co{xchg()}
operation, thus providing the needed update-side spatial synchronization.
If \clnref{if} determines that the pointer to the old structure was
in fact \co{NULL}, \clnref{sr} waits for a grace period (thus providing
the needed update-side temporal synchronization) and \clnref{free}
...
and therefore the return from that same \co{synchronize_rcu()} must
follow the corresponding \co{rcu_read_unlock()}.
...
In short, RCU attains its read-side performance and scalability through
semantics based on combined temporal and spatial synchronization.
...
assigns to \co{p} cannot possibly end until after we reach the matching
\co{rcu_read_unlock()}.
...
The remainder of this section looks at ways of addressing this problem,
while also reducing the complexity of traditional reference counting,
at least in some cases.\footnote{
