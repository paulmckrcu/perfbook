...
@@@ move portions of the following to the locking chapter.
...
\label{fig:defer:Per-Element Locking Without Existence Guarantees}
\end{figure}
...
\label{fig:defer:Per-Element Locking With Lock-Based Existence Guarantees}
\end{figure}

One way to fix this example is to use a hashed set of global locks, so
that each hash bucket has its own lock, as shown in
Figure~\ref{fig:defer:Per-Element Locking With Lock-Based Existence Guarantees}.
