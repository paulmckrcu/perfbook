...
\begin{figure}
\centering
\resizebox{3in}{!}{\includegraphics{defer/hazptr}}
\caption{Reference Counting vs.\@ Hazard Pointers}
\label{fig:defer:Reference Counting vs. Hazard Pointers}
\end{figure}

This arrangement is illustrated by
\cref{fig:defer:Reference Counting vs. Hazard Pointers},
in which the left side illustrates conventional reference counting
(using a \co{->refctr} field within the object), and with the right
side illustrating hazard pointers, in which a per-CPU
pointer references a specific structure or is the \co{NULL} pointer.
In both cases, the corresponding object has two references, one from
CPU~0 and the other from CPU~2.

The biggest advantage of hazard pointers over conventional reference
counting is that updating hazard pointers is a low-cost CPU-local
operation.
In contrast, updating traditional reference counters often requires
costly atomic operations that are subject to expensive cache misses.
Hazard pointers can thus be thought of as a scalable replacement for
reference counting~\cite{McKenney:2013:SDS:2483852.2483867}.
But nothing comes for free, and the corresponding hazard-pointers penalty
is that there is no efficient low-latency mechanism for determining when
the number of hazard-pointer references to a given object has decreased
to zero.
...
Of course, this also means that hazard-pointer acquisition must be
carried out quite carefully in order to avoid destructive races with
concurrent deletion.
...
into C++26~\cite{MagedMichael2023C++26Hazptr3}, and continued refinements
are in
progress~\cite{MagedMichael2025C++29hazptrSyncRec,MagedMichael2025C++29hazptrBatches}.
