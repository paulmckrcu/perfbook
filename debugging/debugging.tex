...
to userspace~\cite{BrendanGregg2018BPFperftools}.
...
is that their overhead rules out production use.
...
therefore provides a \co{lockdep_assert_held()} function that allows
functions to verify that the proper locks are held.
Of course, lockdep incurs significant overhead, so that it is not
necessarily appropriate for production use.
...
job is to resolve any resulting conflicts and of course to take notes.
...
code at hand.
That said, one advantage of the Linux kernel community approach over
...
by the author's invalid assumptions.
Another advantage is that community members can and do test the code.

\QuickQuiz{
	The concerns about invalid assumptions are overblown.
	After all, don't most Linux kernel hackers know what they
	are doing?
	Just what sorts of invalid assumptions are you accusing them
	of harboring, anyway???
}\QuickQuizAnswer{
	...
	Those who look at these commits in greater detail should conclude
...
\item	At each step, articulate and question your assumptions, where
	possible, inserting assertions or constructing tests to check
	them.
