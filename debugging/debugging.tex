...
Finally, given that performance and scalability are first-class requirements
for parallel programming, it is only fitting that the last word is
given to
...
organizations.
One can only hope that the successful organization maintains a suitable
level of humility, lest they be killed by the next project.
...
\section{When Should Validation Start?}
...
down can help flush out the inevitable flaws.
...
First-of-a-kind projects and projects whose only purpose is learning
and practice can require different approaches to validation, for example,
rapid prototyping.
However, it is important to keep in mind that you should not omitting
validation, but rather taking a radically different approach to it.
...
filesystems, it is unlikely that they would have found the bugs.
...
allowing us to dispense with the denominator:
...
Because $m$ is small compared to $n$, we can cancel the factors in
...
Suppose that a bug causes testing to fail about once every hour.
How long must the tests run error-free on that bug's fix before
we can be 99\% certain that the fix has actually improved matters?

To solve this, we set Equation~\ref{eq:debugging:Poisson Probability}
to 0.01 and solve for $\lambda$, resulting in:

\begin{equation}
	\lambda = log 0.01 = 4.6
\end{equation}

We must therefore run error-free for 4.6 hours to be 99\% sure that
our fix has improved matters.
...
\QuickQuiz{}
	...
	of failure?
\QuickQuizAnswer{
	We set Equation~\ref{eq:debugging:Poisson Probability} to 0.001
	and solve for $\lambda$, resulting in:

	\begin{equation}
		\lambda = \log 0.001 = 6.9
	\end{equation}

	The average time until failure was one-third of an hour, so we
	divide 6.9 by 3 yielding 2.3 hours.
} \QuickQuizEnd
...
Here $m$ is the maximum number of errors in the long test run
(in this case, two) and $\lambda$ is expected number of errors
in the long test run (in this case, 24).
Pluggin $m=2$ and $\lambda=24$ into this expression gives about
$1.2*10^{-8}$, indicating that the odds are extremely good that
the fix had a statistically significant effect.
...
The next section discusses ways of handling this situation.
...
However, the field of particle physics inspires another thought:
...
the first place.
This is a bit of a dark art, but there are @@@

Corrupted variables -- check synchronization.

Vary software and hardware configuration and look for statistically
significant differences in failure rate.
This can narrow down the problem considerably.

One important software configuration variable is the history of
...
\subsubsection{Simulate Abnormal Events}
\label{sec:debugging:Simulate Abnormal Events}
