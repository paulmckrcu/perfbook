...
For example, suppose your code is controlling a robot, for example,
making is walk.
The robot's falling over might constitute a bug in your program, in
which case the robot's stumbling and recovering might constitute a
near miss.
...
a too-short time delay between a pair of operations that are supposed
to be protected by the same lock, this too-short time delay could be
counted as a near miss.\footnote{
	Of course, in this case, you would do well to make good use
	of whatever \co{lock_held()} primitive might be available
	in your environment.}

For example, a low-probability bug in RCU priority boosting occurred
roughly once per hundred hours of focused rcutorture testing.
Because it would take almost 500 hours of failure-free testing to be
99\% certain that the bug was not present, the \co{git bisect} process
...
an RCU grace period, but also a second wait for an RCU callback to be
invoked after completion of the RCU callback.
Even more fortunately, \co{rcutorture} includes some statistics that
are sensitive to the wait for grace period without the wait for callback
invocation.
These statistics are subject to false positives due to their unsynchronized
access to RCU's state variables,
