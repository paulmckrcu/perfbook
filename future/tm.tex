...
\item	Use TM strictly as an optimization in lock-based systems, as was
	...
	by a great many transactional lock elision projects.
...
\item	Use TM strictly as an optimization in lock-based systems, as was
	...
	and as has been done by more recent TM reader-writer-lock
	elision work~\cite{PascalFelber2016rwlockElision}.
	This approach seems sound, but leaves the locking design
	constraints (such as the need to avoid deadlock) firmly in place.
\end{enumerate}
...
\QuickQuiz{
	...
	\item	They use a poorly tuned 1,000-bucket hash table containing
		10,000~elements.
		In addition, 448~hardware threads need considerably more
		than 1,000~buckets to avoid the lock contention that
		the correctly note limits RCU in their benchmarks.
	...
	\item	Their RCU hash table used per-bucket locks, which they
		...
		bottleneck, which cynics might claim to shed some light
	...
	\item	Their linked-list operation permits RLU to do concurrent
		...
		with lockless updaters, a fact that has appeared in
		the archival-quality academic literature, and which the
		authors in fact cited~\cite{MathieuDesnoyers2012URCU}.
	...
	scalability combined with strong read-side coherence, even if
	they did choose to hobble their competition.
	They are also to be congratulated on overcoming the traditional
	academic prejudice against asynchronous grace periods and against
	blocking update completion until the grace period completes,
	which did much to promote their scalability.
}\QuickQuizEnd
...
fig:future:The STM Reality: Realtime Response}.\footnote{
	...
	albeit without any performance results.}
