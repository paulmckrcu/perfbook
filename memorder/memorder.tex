...
intuitinos and with more sophisticated rules of thumb.
...
this book is primarily about code that needs multiple CPUs, 
...
\begin{enumerate}
\item	The fact that CPU~0's unlock precedes CPU~1's lock ensures
	that any accesses executed by CPU~0 within its critical
	section and before that critical section will be seen by any
	accesses executed by CPU~1 within its critical section and after
	that critical section.
\item	The fact that CPU~0's unlock precedes CPU~2's lock ensures
	that any accesses executed by CPU~0 within its critical
	section and before that critical section will be seen by any
	accesses executed by CPU~2 within its critical section and after
	that critical section.
\item	The fact that CPU~1's unlock precedes CPU~2's lock ensures
	that any accesses executed by CPU~1 within its critical
	section and before that critical section will be seen by any
	accesses executed by CPU~2 within its critical section and after
	that critical section.
\end{enumerate}
...
so that everything before an earlier lock release is order against
everything after a later lock acquisition.
...
transitively.
And if such code fails to see the accesses in a given critical section,
it will also fail to see the accesses in all of that CPU's
code following that critical section.
...
\item	If A is a store and B is a load, then B will return either the
	value stored by A some later value.
...
Locking is strongly intuitive, which is one reason why it has persisted
despite so many attempts to eliminate it.
...
\item	Note also that CPU~0's release of A is read by CPU~1's acquire of
	...
	acquire of B.  All this ensures that any accesses executed by
...
For those who prefer words to diagrams, when an acquire access loads
the value stored by a release access, discussed in
\cref{sec:memorder:Release-Acquire Chains},
then the code following that release access will see all accesses
preceding the acquire access.
More precisely, if CPU~0 does an acquire access that loads the value
stored by CPU~1's release access, than all the subsequent accesses
executed by CPU~0 will see the all of CPU~1's accesses prior to its
release access.
...
\subsection{Advanced Rules of Thumb}
\label{sec:memorder:Advanced Rules of Thumb}
...
ordering operations.
This rule of thumb underlies the locking intuitions presented in
\cref{sec:memorder:Locking Intuitions}.
