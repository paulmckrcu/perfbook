...
synchronization.
Nevertheless, some argue that weakness is a
virtue~\cite{JadeAlglave2013-WeaknessIsVirtue}.
This chapter will help you gain an understanding of memory ordering,
that, with practice, will be sufficient to implement synchronization
primitives and performance-critical code.
...
to happen after CPU~0's access X1.
...
instead provid the slightly weaker
...
starting on
page~\pageref{sec:toolsoftrade:A Volatile Solution}{sec:toolsoftrade:Avoiding Data Races}:
...
\QuickQuiz{
	...
	preceding and the other following the critical section.
...
\begin{figure*}[tbp]
\centering
\resizebox{0.75\textwidth}{!}{\includegraphics{memorder/RCU1G2Rmb}}
...
\item	[\tco{isync}] forces all preceding instructions to appear to have
	...
	stores do not necessarily force flushing of the store buffers.
\end{description}
...
that is, z~Systems is fully multicopy atomic, and is the only
commercially available system with this property.
