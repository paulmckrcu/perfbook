...
Code holding a given lock will see the accesses in all prior critical
sections for that same lock, transitively.
And if such code sees the accesses in a given critical section, it will
also see the accesses in all of that thread's (or CPU's) code preceding
that critical section.
In other words, when a thread or CPU releases a given lock, all
of that lock's subsequent critical sections will see the accesses in
all of that thread's or CPU's code preceding that lock release.
...
it will also fail to see the accesses in all of that thread's (or CPU's)
code following that critical section.
In other words, when a thread or CPU acquires a given lock, all of
that lock's previous critical sections will be protected from seeing
the accesses in all of that thread's or CPU's code following that lock
acquisition.
...
If a CPU's or thread's code prior to its release of a given lock contains
an access A to a given variable, then for an access B to that same variable
contained in any CPU's or thread's code following a later acquisition
of that same lock:
