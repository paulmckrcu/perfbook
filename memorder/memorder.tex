...
\item	Because \co{P2()}'s write to \co{x2} precedes the end of
	...
	read from \co{x0} to precede the beginning of that grace period.
...
Therefore, the cycle is forbidden: RCU read-side critical sections
cannot span full RCU grace periods.

Which suggests the general rule, which is:  In these sorts of RCU-only
...
are RCU read-side critical sections, the cycle is forbidden.\footnote{
	...
	of LKMM, the fundamental property of RCU implies this seemingly
	more general result~\cite{Alglave:2018:FSC:3173162.3177156}.}
...
But suppose that \co{P0()}'s \co{WRITE_ONCE()} is replaced by an
...
pair of accesses.
In this new litmus test, both \co{P1()} and \co{P2()} would have
critical sections extending beyond the end of \co{P0()}'s grace
period, which in turn would prevent \co{P2()}'s read from \co{x0}
from preceding \co{P0()}'s write.
