...
Figure~\ref{fig:memorder:Cumulativity},
the figure's ``Store X0'' corresponds to the \co{WRITE_ONCE()} on line~13 of
Listing~\ref{lst:memorder:WWC Litmus Test With Release},
the ``Load X1'' to line~20,
the ``Memory Reference Y0'' to line~21,
the ``Memory Reference Y1'' to line~28, and
the ``Memory Reference X2'' to line~29.
...
% @@@ store, release -> acquire, store -> load, smp_mb(), load
