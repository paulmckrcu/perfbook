...
If all your thread-to-thread communication uses store-to-load links
...
links, release-acquire links for all the store-to-load
links suffice.
...
usually substitute \co{smp_wmb()} and \co{WRITE_ONCE()} for
\co{smp_store_release()} on the one hand,
and \co{smp_rmb()} and \co{READ_ONCE()} for \co{smp_load_acquire()}
on the other.

If two or more of your thread-to-thread communications use something other
than store-to-load links, you will need at least one strong barrier
for each load-to-store and for each store-to-store link.
