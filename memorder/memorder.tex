...
shows that this pair of counter-temporal links give the hardware considerable
latitude.
...
\cref{lst:memorder:R Litmus Test With Write Memory Barrier (No Ordering)}.
There is no known ``toy'' hardware implementation that can make this
happen, so we will instead look at the sequence of steps that the PowerPC
architecture goes through to make this happen.

The first step in this process is to translate
\cref{lst:memorder:R Litmus Test With Write Memory Barrier (No Ordering)}
...
\Clnref{stores} show the initial \co{WRITE_ONCE()} calls in both columns;
...
In order for this \co{exists} clause to be satisfied, \co{P9()}'s
...
as needed by propagating them to other threads.
...
ordering is that it is not only propagated to other hardware threads,
but these other hardware threads must also acknowledge it.
This two-way communication allows the hardware threads to cooperate
as required to produce strong global ordering.

\begin{figure*}[tbp]
\centering
\resizebox{\textwidth}{!}{\includegraphics{memorder/PPCMEM0.jpg}}
...
\begin{figure*}[tbp]
\centering
\resizebox{\textwidth}{!}{\includegraphics{memorder/PPCMEM1.jpg}}
...
\cref{fig:memorder:PPCMEM Initial R State}.
Click on the "Interactive" button in the lower left, which should produce
a display as shown in
\cref{fig:memorder:PPCMEM First R Step}.

This display has one clickable link in each thread's state, and as the
``Commit'' in each link suggests, these links commit each thread's first
\co{stw} instruction.
...
To demonstrate how the \co{exists} clause can be satisfied:
...
\item	At this point, there should be no clickable links in either
	of the two thread states, but there should be quite a few
	of them up in the ``Storage subsystem state''.
\item	\co{Partial coherence commit: c:W y=1 -> d:W y=2}.
	...
	perhaps due to a shared store buffer.
...
\item	\co{Read i:W x=0}, which loads the value
	zero, thus satisfying the \co{exists} clause.
...
\begin{figure*}[tbp]
\centering
\resizebox{\textwidth}{!}{\includegraphics{memorder/PPCMEMfinal.jpg}}
