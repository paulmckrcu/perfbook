...
In the figure, if CPU~1's reference to Y follows that of CPU~0,
...
Note well that the ordering for both CPU~0 and CPU~1 are required.
The reason for this is that normal memory-ordering instructions
executed by a given CPU affect only that CPU and the memory subsystem,
including the store buffers, caches, and interconnects discussed in
more detail in
\cref{sec:memorder:Why Hardware Misordering?}.
Therefore, if two CPUs need to synchronize with each other, they both
need to execute memory-ordering instructions so that they both interact
properly with the memory subsystem, and thus indirectly with each other.
That is, the two CPUs' memory-ordering instructions must \emph{pair}
with each other,
which is sometimes referred to as \emph{memory-barrier
pairing}.
This memory-barrier pairing can be seen in
\cref{fig:memorder:If-Then Nature of Memory Ordering},
where the orderings provided by CPU~0 and~1 work together to provide
the if-then ordering,
...
two-thread special case of \emph{cycle}.
...
\cref{fig:memorder:Intel Core 2 Architecture Simplified},
adding a second CPU, and showing the interconnect and main memory
results in
...
In other words, annoying though its overhead might be, there are times
when we really need to invoke \co{smp_mb()}.
\end{fcvref}
...
\cref{sec:memorder:Locking}
examines the surprisingly varied semantics of locking,
\cref{sec:memorder:RCU}
digs more deeply into RCU,
\cref{sec:memorder:SRCU}
looks at idiosyncrasies of SRCU, and
\cref{sec:memorder:Asymmetric Memory Barriers}
discusses asymmetric memory barriers.
...
Although SRCU (for sleepable RCU) has properties quite similar to those
of RCU, it differs from RCU in several important ways:
\begin{enumerate*}[(1)]
\item	Linux-kernel SRCU readers can block,
\item	Each \tco{srcu_struct} structure provides a separate SRCU domain, and
\item	SRCU readers neither nest nor flatten.
\end{enumerate*}

Blocking is currently out of scope for most formal-verification tools, so
the following sections will focus on the SRCU domains and the non-nesting
RCU readers.

\subsubsection{SRCU Domains}
\label{sec:memorder:SRCU Domains}

SRCU offers multiple independent domains, one for each \co{srcu_struct}
structure in the system.
This means that a \co{synchronize_srcu()} invoked for one \co{srcu_struct}
structure will not wait for an SRCU reader (a \co{srcu_read_lock()} and
\co{srcu_read_unlock()} pair) invoked for some other \co{srcu_struct}
structure.

\begin{listing}
\input{CodeSamples/formal/herd/C-SRCU-xdomain@whole.fcv}
\caption{SRCU Domains}
\label{lst:memorder:SRCU Domains}
\end{listing}

\Cref{lst:memorder:SRCU Domains}
demonstrates this property.
\co{P0()}'s SRCU read-side critical section spanning lines~11
and~14 uses \co{srcu_struct} structure \co{s1}, but \co{P1()}'s
\co{synchronize_srcu()} on line~20 uses \co{srcu_struct} structure
\co{s2}.
These two processes are therefore completely independent, which means
that \co{P1()}'s \co{synchronize_srcu()} is not guaranteed to wait
on \co{P0()}'s SRCU reader, which in turn means that the \co{exists}
clause on line~25 can be satisfied.

Modifying \co{P1()} to use \co{s1} instead of \co{s2} forbids this outcome.

\subsubsection{SRCU Non-Nesting Readers}
\label{sec:memorder:SRCU Non-Nesting Readers}

RCU readers nest, but as described in
\cref{sec:defer:RCU has a Family of Wait-to-Finish APIs},
\co{srcu_read_lock()} and friends return a value that is passed into
the corresponding \co{srcu_read_unlock()} function (see
\cref{tab:defer:SRCU Markers for Read-Side Critical Sections}
for a list of the friends).
When the value returned by a call to \co{srcu_read_lock()} is passed
to \co{srcu_read_unlock()}, those two calls mark the bounds of an SRCU
read-side critical section, which does not interact with any other SRCU
read-side critical section.

\begin{listing}
\input{CodeSamples/formal/herd/C-SRCU-readers-no-nest@whole.fcv}
\caption{SRCU Readers Don't Nest}
\label{lst:memorder:SRCU Readers Don't Nest}
\end{listing}

\Cref{lst:memorder:SRCU Readers Don't Nest}
demonstrates this property.
The return value from the \co{srcu_read_lock()} on line~12 is passed to
the \co{srcu_read_unlock()} on line~15 and that on line~14 is passed to
that on line~17.
If this was RCU rather than SRCU, this would be interpreted as an outer
read-side critical section spanning lines~12--17 with an (empty) inner
critical section spanning lines~14--15, which would suffice to forbid
the outcome shown in the \co{exists} clause on line~28.

But this is SRCU, so there are two independent SRCU read-side critical
sections, one spanning lines~12--15 and the other spanning lines~14-17.
And the fact that they overlap does not help, as you can see by considering
the following sequence of events:

\begin{enumerate}
\item	\co{P0()} executes line~12, entering its first SRCU
	read-side critical section.
\item	\co{P1()} executes lines~22 and~23, writing the value \co{1}
	to \co{y} and starting its SRCU grace period.
\item	\co{P0()} executes line~13, reading the value \co{1} from \co{y}.
\item	\co{P0()} executes lines~14 and~15, exiting its first critical
	section and entering its second.
\item	\co{P1()} returns from \co{synchronize_srcu()} because \co{P0()}'s
	first critical section has completed.
	This call to \co{synchronize_srcu()} need not wait for \co{P0()}'s
	second critical section because it began after this call to
	\co{synchronize_srcu()}.
\item	\co{P0()} executes lines~16 and~17, reading the value \co{0}
	from \co{x}.
\item	\co{P1()} executes line~24, writing the value \co{1} to \co{x},
	but too late to affect the value read by \co{P0()}.
\item	\co{P0()} executes line~17, exiting its second critical section.
\end{enumerate}

This straightforward sequence of events satisfies the \co{exists} clause,
as can be confirmed by \co{herd7}.
Of course, removing lines~14 and~15 and replacing line~17's \co{r4} with
\co{r3} forbids this outcome, which also can be confirmed by \co{herd7}.
Finally, converting this litmus test from SRCU to RCU would also forbid
this outcome.

% @@@ When augmented by \co{smp_mb__after_srcu_read_unlock()}.

\subsection{Asymmetric Memory Barriers}
\label{sec:memorder:Asymmetric Memory Barriers}

Given that reader-writer locking and RCU are asymmetric, both favoring
readers, and given that the overhead of the \co{smp_mb()} full memory
barrier is too high for some use cases, it is only reasonable to ask
if there are asymmetric memory barriers with lightweight API members
for fastpaths.
...
\begin{figure}
\centering
\resizebox{2.5in}{!}{\includegraphics{memorder/SystemArchSB-smb}}
\caption{Symmetric Memory Barriers Interactions}
...
on
\cpageref{fig:memorder:If-Then Nature of Memory Ordering}
that a given CPU's memory-barrier instructions interact only with the
memory subsystem (store buffers, caches, and interconnects), but not
directly with other CPUs.
This means that for two CPUs to synchronize with each other, each must
execute a memory-barrier instruction, as shown for CPUs~0 and~1 in
\cref{fig:memorder:Symmetric Memory Barriers Interactions}.
This should not be a surprise given the need for memory-barrier pairing
and by its generalization, cycles.

\QuickQuiz{
	\Cref{fig:memorder:Symmetric Memory Barriers Interactions}
	shows memory barriers interacting with the store buffer, the
	cache, interconnects, and memory.
	But don't light-weight memory-ordering operations (load-acquire,
	store-release, and the PowerPC \co{lwsync} instruction) interact
	only with the store buffer?
}\QuickQuizAnswer{
	Very often, they do indeed, as shown in
	\cref{fig:memorder:Light-Weight Symmetric Memory Barriers Interactions}.

	\begin{figure}
	\centering
	\resizebox{2.5in}{!}{\includegraphics{memorder/SystemArchSB-slmb}}
	\caption{Light-Weight Symmetric Memory Barriers Interactions}
	\label{fig:memorder:Light-Weight Symmetric Memory Barriers Interactions}
	\end{figure}

	However, true though this might often be, it is an
	oversimplification.
	The only reason that light-weight memory-ordering operations can
	get away with interacting only with the store buffer is that the
	cache-coherence protocol propagates these interactions into the
	cache, which in turn propagates the required ordering through
	the interconnect and, if needed, to memory.
}\QuickQuizEnd

\begin{figure}
\centering
\resizebox{2.5in}{!}{\includegraphics{memorder/SystemArchSB-amb}}
\caption{Asymmetric Memory Barriers Interactions}
\label{fig:memorder:Asymmetric Memory Barriers Interactions}
\end{figure}

The trick behind \emph{asymmetric memory barriers} is to create
a \emph{heavy} memory barrier that, when executed by a given CPU,
interacts not only with the memory subsystem, but also directly with
the target CPUs, for example, by sending \IXacrfpl{ipi} to those CPUs.
These target CPUs then interact with the memory subsystem, which on
many architectures happens just by virtue of the IPI being handled.
This is depicted by
\cref{fig:memorder:Asymmetric Memory Barriers Interactions},
which shows CPU~0 executing a memory-barrier instruction but also
sending an IPI to CPU~1.
When CPU~1 executes the IPI handler, it also executes a memory-barrier
instruction, thus providing the needed memory-barrier pairing, but
without requiring CPU~1 to execute any memory-barrier instructions of
its own accord.
CPU~0's memory barrier combined with its IPI (and thus with CPU~1's
involuntary memory barrier) comprise the heavy memory barrier.

However, CPU~1 cannot be completely passive because the compiler can
reorder memory-reference operations, which could destroy ordering.
CPU~1's code must therefore include a Linux-kernel \co{barrier()}
compiler directive, which is termed a \emph{light} memory barrier.

A heavy asymmetric memory barrier can pair with a symmetric memory barrier
(or any other operation that provides sufficient ordering) on the one hand,
or either a heavy or light asymmetric memory barrier on the other.
In other words, a heavy asymmetric memory barrier can pair with everything
that a symmetric memory barrier can pair with, and it can also pair with
light asymmetric memory barriers.
However, a light asymmetric memory barrier can pair \emph{only} with a
heavy memory barrier.
% @@@ Double-check this with litmus tests.

Many systems provide userspace access to heavy asymmetric memory barriers.
For example, Linux provides the \co{membarrier()} system
call~\cite{Corbet2010membarrier}.
But to be fair, some years before that back in 2008, Windows started
providing the \co{FlushProcessWriteBuffers()} system call.

\QuickQuiz{
	But isn't RCU an example of an asymmetric memory barrier
	going way back before the year 2000, with the heavy barrier
	provided by \co{synchronize_rcu()} and the light barrier being
	implicit anywhere you want between \co{rcu_read_lock()} and
	\co{rcu_read_unlock()}?
}\QuickQuizAnswer{
	Kinda?

	If you take that viewpoint, you could think of asymmetric memory
	barriers as being an optimization for a specific RCU use case.
	After all, unlike \co{synchronize_rcu()}, a heavy asymmetric
	barrier need not wait for RCU quiescent states.

	But if you want to go back before 2008, a 2001 Linux-kernel
	patch implemented asymmetric memory barriers for the special case of
	maintaining address dependencies on DEC Alpha~\cite{McKenney01f}.
	And there are unsubstantiated rumors of DEC's Digital UNIX using
	asymmetric memory barriers back in the 1990s.

	But we likely need to face the fact that the invention of the
	asymmetric memory barrier has been lost in the sands of time.
}\QuickQuizEnd
...
Currently, there are different asymmetric-barrier APIs in different
environments, but there is a proposal to add a specific asymmetric-fence
API to the C++ standard~\cite{DavidGoldblatt2022asymmetricFences}.
...
provided by the Linux kernel's \co{sys_membarrier()} system call, thus
being an early example of the asymmetric memory barriers discussed in
\cref{sec:memorder:Asymmetric Memory Barriers}.
