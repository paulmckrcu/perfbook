...
The column labeled ``C'' indicates cumulativity and/or propagation,
and is explained in
Sections~\ref{sec:memorder:Cumulativity}
and~\ref{sec:memorder:Propagation}.
In the meantime, this column can be ignored unless there are three or
more threads involved.
...
of memory ordering.
To begin building such an understanding,
the next section will introduce trick-and-trap litmus tests using
primitives called out in the table, using these tests to further
illuminate memory ordering.
...
\ref{sec:memorder:Non-Single-Copy Atomicity},
...
\subsection{Non-Multicopy Atomicity}
\label{sec:memorder:Non-Multicopy Atomicity}
...
Figure~\ref{lst:memorder:WWC Litmus Test With Dependencies (No Ordering)}
requires \emph{cumulative} ordering, or \emph{cumulativity}.
Given an ordering having the cumulativity property, any stores
from other threads that are seen by any loads that qualify as
...
However, release and acquire operations do provide cumulativity, and
Figure~\ref{lst:memorder:WWC Litmus Test With Release}
substitutes a release operation for
Figure~\ref{lst:memorder:WWC Litmus Test With Dependencies (No Ordering)}'s
data dependency.
@@@
