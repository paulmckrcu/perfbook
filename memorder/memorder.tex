...
The remainder of this section focuses on detailed RCU grace-period
semantics.
...
but at least one of them must be non-zero, courtesy of RCU's fundamental
...
within \co{P1()}'s critical section, so the both-zero prohibition also
applies to the litmus test shown in
...
As can be seen by running \co{herd} on this litmus test, it is
quite possible for both instances of the \co{r1} register to
...
It is again quite possible for both instances of the \co{r1} register to
have final values of 1, as can be verified by running \co{herd} on this
litmus test.
...
\subsection{RCU Update-Side Ordering}
...
At least one of the instances of \co{r2} is guaranteed to be non-zero,
just as would be the case with \co{smp_mb()}.
This should be no surprise given the information presented in
Table~\ref{tab:memorder:Linux-Kernel Memory-Ordering Cheat Sheet}.

\subsection{RCU Readers: Before and After}
...
Keeping that firmly in mind, this section presents some rather exotic
RCU guarantees.
...
Listing~\ref{lst:memorder:RCU Fundamental Property}:
At least one of the two instances of \co{r2} is guaranteed to be non-zero.
...
critical section.
Again, at least one of the \co{r2} registers is guaranteed to have a
non-zero final value, as can be checked with the \co{herd} tool.
...
still manages to guarantee that at least one of the \co{r2} registers
will have a non-zero final value.
...
As can be checked with \co{herd}, this litmus test allows both
instances of \co{r2} to have final values of zero.
...
forbid this outcome.
However, it is necessary to allow for non-QSBR implementations
as well, and these implementations have no way to enforce ordering
in this case.
Again, therefore, litmus test allows both instances of \co{r2} to have
final values of zero.
...
\QuickQuiz{}
	The section header is ``Higher-Level Primitives'', plural.
	So why is only one primitive presented?
\QuickQuizAnswer{
	Patience!
	Other primitives will make their presence known in the fullness
	of time.
} \QuickQuizEnd
