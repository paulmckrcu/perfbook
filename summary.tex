...
\chapter{The Summing Up}
\label{chp:The Summing Up}
%
...
high-level approaches for addressing those challenges.
It also touched on ways of gaining the benefits of parallelism while
avoiding most of its challenges.

\Cref{chp:Hardware and its Habits} gave a high-level overview of multicore
hardware, especially aspects that pose challenges for concurrent software.
...
them.
In the meantime, software architects and engineers must meet these
...
and \cref{chp:Counting} demonstrated use of those tools and of
parallel-programming design on the simple but surprisingly challenging
task of concurrent counting.
...
into perhaps the most important parallel-programming design technique,
partitioning the problem at the highest possible level.
This \lcnamecref{cha:Partitioning and Synchronization Design} gave an
overview of a number of design points in this space.
...
and presented some engineering solutions to the well-known and
...
greatly improve performance and scalabilty, while in a surprisingly large
...
Finally, \cref{chp:Ease of Use} presented a very brief overview of the
surprisingly important topic of ease of use.
...
They went back and forth for some time, for example, the sound-bite answer
``Deadlock'' and the sound-bite response ``Lock dependency checker''.

The panelist eventually ran out of soundbites, and thus improvised a final
...
heard of by the name of Linus Torvalds.
...
He then suggested that \emph{if} tools continued to improve, \emph{maybe}
parallel programming would become routine by the year 2021.

The conference then concluded.
Paul was not surprised to be given wide berth by many audience members,
especially those who saw the world in the same way that as first panelist
saw it.
He was also not surprised that a few came up and thanked him for the
question.
...
You see, this man had worked for a few years at Sequent, and thus very
well understood parallel programming.
And he was currently assigned to a group whose job it was to write
parallel code.
Which was not working out well.
