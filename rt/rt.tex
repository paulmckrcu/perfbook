...
looks at a number of definitions of ``real-time computing,'' sorting
meaning from the usual sound bites.
...
notes that parallel real-time computing is upon us, and asks
who it is that really needs it.
...
\ContributedBy{figure}{fig:rt:Real-Time Response Guarantee, Meet Hammer}{Melissa Broussard}
\end{figure}
...
commodity computer components often refuse to operate at sub-zero
...
on the environment immediately surreounding the computer, a constraint
...
levels shock and vibration, for example, engine control systems.
...
Each of these five phases of operation might well have its own set of
...
That said, choices of hardware, drivers, and software configuration would
need to support phase~4's more severe requirements.
...
and a sense of shared goals are needed to resolve these conflicts in
a timely fashion.
...
As a point of fact, most real-world real-time system use an acceptance
test rather than formal proofs.
...
change this situation, but as of early 2015, large advances are required.
...
In addition to latency requirement for the real-time portions of the
...
For one moderately extreme example, when purchase a birthday gift online,
...
Although the resulting latency requirements can easily be specified
as described in
...
regardless.
Unfortunately, the mathematical basis for real-time
...
hardware to fit the real-time mathematical circle.
...
Of course, some applications will place the polling loop into the kernel,
...
system will of course depend on the deadlines, for example, as shown in
...
Special real-time Java implementation can provide real-time response
...
Java's large standard libraries.)
...
response in virtualized environments~\cite{JanKiszka2014virtRT}.
...
real-time system.
One approach is to port a general-purpose non-realtime OS on top
of a special purpose real-time operating system (RTOS), as shown in
Figure~\ref{fig:rt:Linux Ported to RTOS}.
The green ``Linux Process'' boxes represent non-realtime processes
running in the 
...
Furthermore, each such RTOSes often has its own system-call interface
...
code full access the then rich Linux open-source ecosystem.
...
short-term response during the time that the Linux kernel had essentially
no real-time capabilities, it also motivates adding real-time capabilities
to the Linux kernel.
...
Over time, functionality from the -rt patchset is added to mainline,
...
The preemptible kernels shown in the middle of
Figure~\ref{fig:rt:Linux-Kernel Real-Time Implementations}
...
left-most diagram in the middle of the figure.
...
non-housekeeping CPUs, but instead only recognizes when that other
processing is no longer present.
If configured properly, a non-trivial undertaking, \co{CONFIG_NO_HZ_FULL}
offers real-time processes very nearly bare-metal levels of performance.

Some has of course been much debate over which of these approaches
...
almost always cancelled before they have a chance to fire),\footnote{
	At least assuming reasonable connectivity!}
timed delays (as in \co{sleep(1)}, which are rarely cancelled),
and timeouts for the \co{poll()} system call (which are usually
...
long-duration timeouts (for example, keepalive timeouts for TCP
...
\begin{figure}[tb]
\begin{center}
\resizebox{3.0in}{!}{\includegraphics{rt/timerwheel}}
\end{center}
\caption{Timer Wheel}
...
next four bits (currently (0x1) index the next level up.
...
provides acceptably find timer granularity for most (but not all!)
...
deliver large number of events with a single interrupt, which means
...
a group of medium-priority threads, at least one per CPU.
...
The priority-inheritance solution has the high-priority thread attempting
...
Another example limitation is that it addresses only lock-based priority
inversions within the context of a given operating system.
One priority-inversion scenario that it cannot address is if a high-priority
thread is waiting on a network socket for a message that is to be written
by a low-priority process that is preempted by a set of CPU-bound
medium-priority processes.
In addition, one potential disadvantage of use of priority inheritance
for situations involving user input is fancifully depicted in
...
\item	Only allow $N$ read-acquisitions of a given reader-writer lcok
...
\item	Prohibit high-priority threads from write-acquiring reader-writer
	locks that are every read-acquired by threads running at lower
	priorities.
...
and later updated to a simplified and more efficient implementation.
...
been preempted within their current RCU read-side critical section.
...
This functionality has not yet reached mainline: Although they is a conceptually
simple substitution of sleeplocks for spinlocks, they has proven relatively
...
simply not to have a kernel, and many real-time applications have
indeed run on bare metal, particularly on eight-bit microcontrollers.
...
Unfortunately, the reality is more complex, however, it is becoming
possible to do just that, at least on recent versions of the Linux
kernel~\cite{JonCorbet2013NO-HZ-FULL}.
Nevertheless, considerable care is required to properly set up such
an environment, as it is not normally possible to counteract any
OS jitter that arises.
It is instead necessary to prevent it at each possible source,
of which there are many.
The discussion below covers interrupts, kernel threads and daemons, @@@

For example, interrupts are an excellent way to introduce large amounts of
OS jitter, thus spoiling bare-metal performance.
However, in most cases interrupts are absolutely required in order
...
The \co{Documentation/IRQ-affinity.txt} file in the Linux source tree
describes how to direct device interrupts to specified CPU,
which as of this writing involves something like the following:
...
The scheduling-clock interrupts require special handling, and are
...
Some of these kthreads require either that certain types of hardware
...
\co{Documentation} directory at \co{kernel-per-CPU-kthreads.txt}.
...
important non-realtime work is allotted at least 50 milliseoncds
...
and do not forget the Spiderman principle.
One way to minimize the risks is to offload interrupts and
kernel threads/daemons from all CPUs running CPU-bound realtime
threads, as described in the paragraphs above.
In addition, you should carefully read the material in the
\co{Documentation/scheduler} directory.
The material in the \co{sched-rt-group.txt} file is particularly
important, especially if you are the \co{cgroups} real-time features
enabled by the \co{CONFIG_RT_GROUP_SCHED} Kconfig parameter, in which
case you should also read the material in the
\co{Documentation/cgroups} directory as well.
...
workloads and fix any real-time bugs.\footnote{
	...
	so that others can beeefit.
	Keep in mind that when you need to port your application to
	a later version of the Linux kernel, \emph{you} will be one of these
	``others.''}
...
At the time of this writing (early 2015), the way to avoid in-kernel
...
as worker CPUs, thus leaving CPUs~1 and~1 as housekeeping CPUs.
...
found in \co{Documentation/timers/NO_HZ.txt}.
...
of OS jitter.
Given a CPU $N$ running a CPU-bound usermode thread, the following
commands will produce a list of all the times that this CPU entered
the kernel:

\begin{quote}
	\scriptsize
	\co{cd /sys/kernel/debug/tracing}
	\co{echo 1 > max_graph_depth # Increase the "1" for more detail}
	\co{echo function_graph > current_tracer}
	\co{# run workload}
	\co{cat per_cpu/cpuN/trace}
\end{quote}

Of course, the \co{N} in the last command must be replaced with the
number of the CPU in question.
...
expected to appear relatively sloely.
Nevertheless, the ability to gain near-bare-metal performance whlie
...
bugs~\cite{DanAlitarh2013PracticalProgress}.\footnote{
	...
	progress}, which is a welcome step towards real-time practice.}
