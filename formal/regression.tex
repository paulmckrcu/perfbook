...
The following sections discuss each of the above requirements.
...
\subsection{Formal Regression Scorcard}
\label{sec:formal:Formal Regression Scorcard}
...
It has reasonable overhead (for a formal-verification tool, anyway)
...
is not quite as helpful as a traceback, but is still quite useful, so its
fourth cell is yellow.
It requires constructing an \co{exists} clause, but cannot take
intra-process assertion, so its fifth cell is also yellow.
...
Its overhead is quite low (for a formal-verification tool), so its
third cell is green.
It provides a traceback, so its fourth cell is green.
It takes assertions directly from the C code, so its fifth cell is blue.
