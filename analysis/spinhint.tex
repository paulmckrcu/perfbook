% analysis/spinhint.html

\section{Using Promela and Spin to Verify Parallel Algorithms}
\label{sec:analysis:Using Promela and Spin to Verify Parallel Algorithms}
...
\subsection{What are Promela and Spin?}
\label{sec:analysis:What are Promela and Spin?}
...
book on Promela and Spin, as well as online references starting at:
...
\subsection{Promela Example: Non-Atomic Increment}
\label{sec:analysis:Promela Example: Non-Atomic Increment}
...
\subsection{Promela Example: Atomic Increment}
\label{sec:analysis:Promela Example: Atomic Increment}
...
\label{sec:analysis:Combinatorial Explosion}
...
Running unnecessarily large models is thus subtlely discouraged, although
...
\label{sec:analysis:How to Use Promela}
...
\label{sec:analysis:Promela Peculiarities}
...
\label{sec:analysis:Promela Coding Tricks}
...
\label{sec:analysis:Promela Example: Locking}
...
\QuickQuiz{Why is there an unreached statement in
...
\QuickQuiz{What are some Promela code-style issues with this example?}{
There are several: \\
(1)	The declaration of {\tt sum} should be moved to within
	the init block, since it is not used anywhere else. \\
(2)	The assertion code should be moved outside of the
...
(3)	The atomic block covering the assertion code should
...
\label{sec:analysis:Promela Example: QRCU}

This final example demonstrates a real-world use of Promela on Oleg
Nesterov's QRCU, but modified to speed up the \url{synchronize_qrcu()}
fastpath.
...
\QuickQuiz{Is there a more straightforward way to code the do-od statement?}{
...
\QuickQuiz{Why are there atomic blocks at lines 12-21
...
\QuickQuiz{Is the re-summing of the counters on lines 24-27
...
(2)	An updater starts executing, and sees that the sum of
	the counters is two so that the fastpath cannot be
	executed.  It therefore acquires the lock. \\
(3)	A second updater starts executing, and fetches the value
	of {\tt ctr[0]}, which is zero. \\
(4)	The first updater adds one to {\tt ctr[0]}, flips
	the index (which now becomes zero), then subtracts
	one from {\tt ctr[1]} (which now becomes one). \\
(5)	The second updater fetches the value of {\tt ctr[1]},
	which is now one. \\
(6)	The second updater now incorrectly concludes that it
...
\label{sec:analysis:Running the QRCU Example}
...
\label{sec:analysis:How Many Readers and Updaters Are Really Needed?}
...
\label{sec:analysis:Proof of Correctness}
...
\subsubsection{More Capable Tools}
\label{sec:analysis:More Capable Tools}
...
\label{sec:analysis:Divide and Conquer}
